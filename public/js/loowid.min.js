...
}if(rnumnonpx.test(ret)&&!rposition.test(name)){left=style.left;rsLeft=elem.runtimeStyle&&elem.runtimeStyle.left;if(rsLeft){elem.runtimeStyle.left=elem.currentStyle.left}style.left=name==="fontSize"?"1em":ret;ret=style.pixelLeft+"px";style.left=left;if(rsLeft){elem.runtimeStyle.left=rsLeft}}return ret===""?"auto":ret}}function setPositiveNumber(elem,value,subtract){var matches=rnumsplit.exec(value);return matches?Math.max(0,matches[1]-(subtract||0))+(matches[2]||"px"):value}function augmentWidthOrHeight(elem,name,extra,isBorderBox){var i=extra===(isBorderBox?"border":"content")?4:name==="width"?1:0,val=0;for(;i<4;i+=2){if(extra==="margin"){val+=jQuery.css(elem,extra+cssExpand[i],true)}if(isBorderBox){if(extra==="content"){val-=parseFloat(curCSS(elem,"padding"+cssExpand[i]))||0}if(extra!=="margin"){val-=parseFloat(curCSS(elem,"border"+cssExpand[i]+"Width"))||0}}else{val+=parseFloat(curCSS(elem,"padding"+cssExpand[i]))||0;if(extra!=="padding"){val+=parseFloat(curCSS(elem,"border"+cssExpand[i]+"Width"))||0}}}return val}function getWidthOrHeight(elem,name,extra){var val=name==="width"?elem.offsetWidth:elem.offsetHeight,valueIsBorderBox=true,isBorderBox=jQuery.support.boxSizing&&jQuery.css(elem,"boxSizing")==="border-box";if(val<=0||val==null){val=curCSS(elem,name);if(val<0||val==null){val=elem.style[name]}if(rnumnonpx.test(val)){return val}valueIsBorderBox=isBorderBox&&(jQuery.support.boxSizingReliable||val===elem.style[name]);val=parseFloat(val)||0}return val+augmentWidthOrHeight(elem,name,extra||(isBorderBox?"border":"content"),valueIsBorderBox)+"px"}function css_defaultDisplay(nodeName){if(elemdisplay[nodeName]){return elemdisplay[nodeName]}var elem=jQuery("<"+nodeName+">").appendTo(document.body),display=elem.css("display");elem.remove();if(display==="none"||display===""){iframe=document.body.appendChild(iframe||jQuery.extend(document.createElement("iframe"),{frameBorder:0,width:0,height:0}));if(!iframeDoc||!iframe.createElement){iframeDoc=(iframe.contentWindow||iframe.contentDocument).document;iframeDoc.write("<!doctype html><html><body>");iframeDoc.close()}elem=iframeDoc.body.appendChild(iframeDoc.createElement(nodeName));display=curCSS(elem,"display");document.body.removeChild(iframe)}elemdisplay[nodeName]=display;return display}jQuery.each(["height","width"],function(i,name){jQuery.cssHooks[name]={get:function(elem,computed,extra){if(computed){if(elem.offsetWidth===0&&rdisplayswap.test(curCSS(elem,"display"))){return jQuery.swap(elem,cssShow,function(){return getWidthOrHeight(elem,name,extra)})}else{return getWidthOrHeight(elem,name,extra)}}},set:function(elem,value,extra){return setPositiveNumber(elem,value,extra?augmentWidthOrHeight(elem,name,extra,jQuery.support.boxSizing&&jQuery.css(elem,"boxSizing")==="border-box"):0)}}});if(!jQuery.support.opacity){jQuery.cssHooks.opacity={get:function(elem,computed){return ropacity.test((computed&&elem.currentStyle?elem.currentStyle.filter:elem.style.filter)||"")?.01*parseFloat(RegExp.$1)+"":computed?"1":""},set:function(elem,value){var style=elem.style,currentStyle=elem.currentStyle,opacity=jQuery.isNumeric(value)?"alpha(opacity="+value*100+")":"",filter=currentStyle&&currentStyle.filter||style.filter||"";style.zoom=1;if(value>=1&&jQuery.trim(filter.replace(ralpha,""))===""&&style.removeAttribute){style.removeAttribute("filter");if(currentStyle&&!currentStyle.filter){return}}style.filter=ralpha.test(filter)?filter.replace(ralpha,opacity):filter+" "+opacity}}}jQuery(function(){if(!jQuery.support.reliableMarginRight){jQuery.cssHooks.marginRight={get:function(elem,computed){return jQuery.swap(elem,{display:"inline-block"},function(){if(computed){return curCSS(elem,"marginRight")}})}}}if(!jQuery.support.pixelPosition&&jQuery.fn.position){jQuery.each(["top","left"],function(i,prop){jQuery.cssHooks[prop]={get:function(elem,computed){if(computed){var ret=curCSS(elem,prop);return rnumnonpx.test(ret)?jQuery(elem).position()[prop]+"px":ret}}}})}});if(jQuery.expr&&jQuery.expr.filters){jQuery.expr.filters.hidden=function(elem){return elem.offsetWidth===0&&elem.offsetHeight===0||!jQuery.support.reliableHiddenOffsets&&(elem.style&&elem.style.display||curCSS(elem,"display"))==="none"};jQuery.expr.filters.visible=function(elem){return!jQuery.expr.filters.hidden(elem)}}jQuery.each({margin:"",padding:"",border:"Width"},function(prefix,suffix){jQuery.cssHooks[prefix+suffix]={expand:function(value){var i,parts=typeof value==="string"?value.split(" "):[value],expanded={};for(i=0;i<4;i++){expanded[prefix+cssExpand[i]+suffix]=parts[i]||parts[i-2]||parts[0]}return expanded}};if(!rmargin.test(prefix)){jQuery.cssHooks[prefix+suffix].set=setPositiveNumber}});var r20=/%20/g,rbracket=/\[\]$/,rCRLF=/\r?\n/g,rinput=/^(?:color|date|datetime|datetime-local|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i,rselectTextarea=/^(?:select|textarea)/i;jQuery.fn.extend({serialize:function(){return jQuery.param(this.serializeArray())},serializeArray:function(){return this.map(function(){return this.elements?jQuery.makeArray(this.elements):this}).filter(function(){return this.name&&!this.disabled&&(this.checked||rselectTextarea.test(this.nodeName)||rinput.test(this.type))}).map(function(i,elem){var val=jQuery(this).val();return val==null?null:jQuery.isArray(val)?jQuery.map(val,function(val,i){return{name:elem.name,value:val.replace(rCRLF,"\r\n")}}):{name:elem.name,value:val.replace(rCRLF,"\r\n")}}).get()}});jQuery.param=function(a,traditional){var prefix,s=[],add=function(key,value){value=jQuery.isFunction(value)?value():value==null?"":value;s[s.length]=encodeURIComponent(key)+"="+encodeURIComponent(value)};if(traditional===undefined){traditional=jQuery.ajaxSettings&&jQuery.ajaxSettings.traditional}if(jQuery.isArray(a)||a.jquery&&!jQuery.isPlainObject(a)){jQuery.each(a,function(){add(this.name,this.value)})}else{for(prefix in a){buildParams(prefix,a[prefix],traditional,add)}}return s.join("&").replace(r20,"+")};function buildParams(prefix,obj,traditional,add){var name;if(jQuery.isArray(obj)){jQuery.each(obj,function(i,v){if(traditional||rbracket.test(prefix)){add(prefix,v)}else{buildParams(prefix+"["+(typeof v==="object"?i:"")+"]",v,traditional,add)}})}else if(!traditional&&jQuery.type(obj)==="object"){for(name in obj){buildParams(prefix+"["+name+"]",obj[name],traditional,add)}}else{add(prefix,obj)}}var ajaxLocParts,ajaxLocation,rhash=/#.*$/,rheaders=/^(.*?):[ \t]*([^\r\n]*)\r?$/gm,rlocalProtocol=/^(?:about|app|app\-storage|.+\-extension|file|res|widget):$/,rnoContent=/^(?:GET|HEAD)$/,rprotocol=/^\/\//,rquery=/\?/,rscript=/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,rts=/([?&])_=[^&]*/,rurl=/^([\w\+\.\-]+:)(?:\/\/([^\/?#:]*)(?::(\d+)|)|)/,_load=jQuery.fn.load,prefilters={},transports={},allTypes=["*/"]+["*"];try{ajaxLocation=location.href}catch(e){ajaxLocation=document.createElement("a");ajaxLocation.href="";ajaxLocation=ajaxLocation.href}ajaxLocParts=rurl.exec(ajaxLocation.toLowerCase())||[];function addToPrefiltersOrTransports(structure){return function(dataTypeExpression,func){if(typeof dataTypeExpression!=="string"){func=dataTypeExpression;dataTypeExpression="*"}var dataType,list,placeBefore,dataTypes=dataTypeExpression.toLowerCase().split(core_rspace),i=0,length=dataTypes.length;if(jQuery.isFunction(func)){for(;i<length;i++){dataType=dataTypes[i];placeBefore=/^\+/.test(dataType);if(placeBefore){dataType=dataType.substr(1)||"*"}list=structure[dataType]=structure[dataType]||[];list[placeBefore?"unshift":"push"](func)}}}}function inspectPrefiltersOrTransports(structure,options,originalOptions,jqXHR,dataType,inspected){dataType=dataType||options.dataTypes[0];inspected=inspected||{};inspected[dataType]=true;var selection,list=structure[dataType],i=0,length=list?list.length:0,executeOnly=structure===prefilters;for(;i<length&&(executeOnly||!selection);i++){selection=list[i](options,originalOptions,jqXHR);if(typeof selection==="string"){if(!executeOnly||inspected[selection]){selection=undefined}else{options.dataTypes.unshift(selection);selection=inspectPrefiltersOrTransports(structure,options,originalOptions,jqXHR,selection,inspected)}}}if((executeOnly||!selection)&&!inspected["*"]){selection=inspectPrefiltersOrTransports(structure,options,originalOptions,jqXHR,"*",inspected)}return selection}function ajaxExtend(target,src){var key,deep,flatOptions=jQuery.ajaxSettings.flatOptions||{};for(key in src){if(src[key]!==undefined){(flatOptions[key]?target:deep||(deep={}))[key]=src[key]}}if(deep){jQuery.extend(true,target,deep)}}jQuery.fn.load=function(url,params,callback){if(typeof url!=="string"&&_load){return _load.apply(this,arguments)}if(!this.length){return this}var selector,type,response,self=this,off=url.indexOf(" ");if(off>=0){selector=url.slice(off,url.length);url=url.slice(0,off)}if(jQuery.isFunction(params)){callback=params;params=undefined}else if(params&&typeof params==="object"){type="POST"}jQuery.ajax({url:url,type:type,dataType:"html",data:params,complete:function(jqXHR,status){if(callback){self.each(callback,response||[jqXHR.responseText,status,jqXHR])}}}).done(function(responseText){response=arguments;self.html(selector?jQuery("<div>").append(responseText.replace(rscript,"")).find(selector):responseText)});return this};jQuery.each("ajaxStart ajaxStop ajaxComplete ajaxError ajaxSuccess ajaxSend".split(" "),function(i,o){jQuery.fn[o]=function(f){return this.on(o,f)}});jQuery.each(["get","post"],function(i,method){jQuery[method]=function(url,data,callback,type){if(jQuery.isFunction(data)){type=type||callback;callback=data;data=undefined}return jQuery.ajax({type:method,url:url,data:data,success:callback,dataType:type})}});jQuery.extend({getScript:function(url,callback){return jQuery.get(url,undefined,callback,"script")},getJSON:function(url,data,callback){return jQuery.get(url,data,callback,"json")},ajaxSetup:function(target,settings){if(settings){ajaxExtend(target,jQuery.ajaxSettings)}else{settings=target;target=jQuery.ajaxSettings}ajaxExtend(target,settings);return target},ajaxSettings:{url:ajaxLocation,isLocal:rlocalProtocol.test(ajaxLocParts[1]),global:true,type:"GET",contentType:"application/x-www-form-urlencoded; charset=UTF-8",processData:true,async:true,accepts:{xml:"application/xml, text/xml",html:"text/html",text:"text/plain",json:"application/json, text/javascript","*":allTypes},contents:{xml:/xml/,html:/html/,json:/json/},responseFields:{xml:"responseXML",text:"responseText"},converters:{"* text":window.String,"text html":true,"text json":jQuery.parseJSON,"text xml":jQuery.parseXML},flatOptions:{context:true,url:true}},ajaxPrefilter:addToPrefiltersOrTransports(prefilters),ajaxTransport:addToPrefiltersOrTransports(transports),ajax:function(url,options){if(typeof url==="object"){options=url;url=undefined}options=options||{};var ifModifiedKey,responseHeadersString,responseHeaders,transport,timeoutTimer,parts,fireGlobals,i,s=jQuery.ajaxSetup({},options),callbackContext=s.context||s,globalEventContext=callbackContext!==s&&(callbackContext.nodeType||callbackContext instanceof jQuery)?jQuery(callbackContext):jQuery.event,deferred=jQuery.Deferred(),completeDeferred=jQuery.Callbacks("once memory"),statusCode=s.statusCode||{},requestHeaders={},requestHeadersNames={},state=0,strAbort="canceled",jqXHR={readyState:0,setRequestHeader:function(name,value){if(!state){var lname=name.toLowerCase();name=requestHeadersNames[lname]=requestHeadersNames[lname]||name;requestHeaders[name]=value}return this},getAllResponseHeaders:function(){return state===2?responseHeadersString:null},getResponseHeader:function(key){var match;if(state===2){if(!responseHeaders){responseHeaders={};while(match=rheaders.exec(responseHeadersString)){responseHeaders[match[1].toLowerCase()]=match[2]}}match=responseHeaders[key.toLowerCase()]}return match===undefined?null:match},overrideMimeType:function(type){if(!state){s.mimeType=type}return this},abort:function(statusText){statusText=statusText||strAbort;if(transport){transport.abort(statusText)}done(0,statusText);return this}};function done(status,nativeStatusText,responses,headers){var isSuccess,success,error,response,modified,statusText=nativeStatusText;if(state===2){return}state=2;if(timeoutTimer){clearTimeout(timeoutTimer)}transport=undefined;responseHeadersString=headers||"";jqXHR.readyState=status>0?4:0;if(responses){response=ajaxHandleResponses(s,jqXHR,responses)}if(status>=200&&status<300||status===304){if(s.ifModified){modified=jqXHR.getResponseHeader("Last-Modified");if(modified){jQuery.lastModified[ifModifiedKey]=modified}modified=jqXHR.getResponseHeader("Etag");if(modified){jQuery.etag[ifModifiedKey]=modified}}if(status===304){statusText="notmodified";isSuccess=true}else{isSuccess=ajaxConvert(s,response);statusText=isSuccess.state;success=isSuccess.data;error=isSuccess.error;isSuccess=!error}}else{error=statusText;if(!statusText||status){statusText="error";if(status<0){status=0}}}jqXHR.status=status;jqXHR.statusText=(nativeStatusText||statusText)+"";if(isSuccess){deferred.resolveWith(callbackContext,[success,statusText,jqXHR])}else{deferred.rejectWith(callbackContext,[jqXHR,statusText,error])}jqXHR.statusCode(statusCode);statusCode=undefined;if(fireGlobals){globalEventContext.trigger("ajax"+(isSuccess?"Success":"Error"),[jqXHR,s,isSuccess?success:error])}completeDeferred.fireWith(callbackContext,[jqXHR,statusText]);if(fireGlobals){globalEventContext.trigger("ajaxComplete",[jqXHR,s]);if(!--jQuery.active){jQuery.event.trigger("ajaxStop")}}}deferred.promise(jqXHR);jqXHR.success=jqXHR.done;jqXHR.error=jqXHR.fail;jqXHR.complete=completeDeferred.add;jqXHR.statusCode=function(map){if(map){var tmp;if(state<2){for(tmp in map){statusCode[tmp]=[statusCode[tmp],map[tmp]]}}else{tmp=map[jqXHR.status];jqXHR.always(tmp)}}return this};s.url=((url||s.url)+"").replace(rhash,"").replace(rprotocol,ajaxLocParts[1]+"//");s.dataTypes=jQuery.trim(s.dataType||"*").toLowerCase().split(core_rspace);if(s.crossDomain==null){parts=rurl.exec(s.url.toLowerCase())||false;s.crossDomain=parts&&parts.join(":")+(parts[3]?"":parts[1]==="http:"?80:443)!==ajaxLocParts.join(":")+(ajaxLocParts[3]?"":ajaxLocParts[1]==="http:"?80:443)}if(s.data&&s.processData&&typeof s.data!=="string"){s.data=jQuery.param(s.data,s.traditional)}inspectPrefiltersOrTransports(prefilters,s,options,jqXHR);if(state===2){return jqXHR}fireGlobals=s.global;s.type=s.type.toUpperCase();s.hasContent=!rnoContent.test(s.type);if(fireGlobals&&jQuery.active++===0){jQuery.event.trigger("ajaxStart")}if(!s.hasContent){if(s.data){s.url+=(rquery.test(s.url)?"&":"?")+s.data;delete s.data}ifModifiedKey=s.url;if(s.cache===false){var ts=jQuery.now(),ret=s.url.replace(rts,"$1_="+ts);s.url=ret+(ret===s.url?(rquery.test(s.url)?"&":"?")+"_="+ts:"")}}if(s.data&&s.hasContent&&s.contentType!==false||options.contentType){jqXHR.setRequestHeader("Content-Type",s.contentType)}if(s.ifModified){ifModifiedKey=ifModifiedKey||s.url;if(jQuery.lastModified[ifModifiedKey]){jqXHR.setRequestHeader("If-Modified-Since",jQuery.lastModified[ifModifiedKey])}if(jQuery.etag[ifModifiedKey]){jqXHR.setRequestHeader("If-None-Match",jQuery.etag[ifModifiedKey])}}jqXHR.setRequestHeader("Accept",s.dataTypes[0]&&s.accepts[s.dataTypes[0]]?s.accepts[s.dataTypes[0]]+(s.dataTypes[0]!=="*"?", "+allTypes+"; q=0.01":""):s.accepts["*"]);for(i in s.headers){jqXHR.setRequestHeader(i,s.headers[i])}if(s.beforeSend&&(s.beforeSend.call(callbackContext,jqXHR,s)===false||state===2)){return jqXHR.abort()}strAbort="abort";for(i in{success:1,error:1,complete:1}){jqXHR[i](s[i])}transport=inspectPrefiltersOrTransports(transports,s,options,jqXHR);if(!transport){done(-1,"No Transport")}else{jqXHR.readyState=1;if(fireGlobals){globalEventContext.trigger("ajaxSend",[jqXHR,s])}if(s.async&&s.timeout>0){timeoutTimer=setTimeout(function(){jqXHR.abort("timeout")},s.timeout)}try{state=1;transport.send(requestHeaders,done)}catch(e){if(state<2){done(-1,e)}else{throw e}}}return jqXHR},active:0,lastModified:{},etag:{}});function ajaxHandleResponses(s,jqXHR,responses){var ct,type,finalDataType,firstDataType,contents=s.contents,dataTypes=s.dataTypes,responseFields=s.responseFields;for(type in responseFields){if(type in responses){jqXHR[responseFields[type]]=responses[type]}}while(dataTypes[0]==="*"){dataTypes.shift();if(ct===undefined){ct=s.mimeType||jqXHR.getResponseHeader("content-type")}}if(ct){for(type in contents){if(contents[type]&&contents[type].test(ct)){dataTypes.unshift(type);break}}}if(dataTypes[0]in responses){finalDataType=dataTypes[0]}else{for(type in responses){if(!dataTypes[0]||s.converters[type+" "+dataTypes[0]]){finalDataType=type;break}if(!firstDataType){firstDataType=type}}finalDataType=finalDataType||firstDataType}if(finalDataType){if(finalDataType!==dataTypes[0]){dataTypes.unshift(finalDataType)}return responses[finalDataType]}}function ajaxConvert(s,response){var conv,conv2,current,tmp,dataTypes=s.dataTypes.slice(),prev=dataTypes[0],converters={},i=0;if(s.dataFilter){response=s.dataFilter(response,s.dataType)}if(dataTypes[1]){for(conv in s.converters){converters[conv.toLowerCase()]=s.converters[conv]}}for(;current=dataTypes[++i];){if(current!=="*"){if(prev!=="*"&&prev!==current){conv=converters[prev+" "+current]||converters["* "+current];if(!conv){for(conv2 in converters){tmp=conv2.split(" ");if(tmp[1]===current){conv=converters[prev+" "+tmp[0]]||converters["* "+tmp[0]];if(conv){if(conv===true){conv=converters[conv2]}else if(converters[conv2]!==true){current=tmp[0];dataTypes.splice(i--,0,current)}break}}}}if(conv!==true){if(conv&&s["throws"]){response=conv(response)}else{try{response=conv(response)}catch(e){return{state:"parsererror",error:conv?e:"No conversion from "+prev+" to "+current}}}}}prev=current}}return{state:"success",data:response}}var oldCallbacks=[],rquestion=/\?/,rjsonp=/(=)\?(?=&|$)|\?\?/,nonce=jQuery.now();jQuery.ajaxSetup({jsonp:"callback",jsonpCallback:function(){var callback=oldCallbacks.pop()||jQuery.expando+"_"+nonce++;this[callback]=true;return callback}});jQuery.ajaxPrefilter("json jsonp",function(s,originalSettings,jqXHR){var callbackName,overwritten,responseContainer,data=s.data,url=s.url,hasCallback=s.jsonp!==false,replaceInUrl=hasCallback&&rjsonp.test(url),replaceInData=hasCallback&&!replaceInUrl&&typeof data==="string"&&!(s.contentType||"").indexOf("application/x-www-form-urlencoded")&&rjsonp.test(data);if(s.dataTypes[0]==="jsonp"||replaceInUrl||replaceInData){callbackName=s.jsonpCallback=jQuery.isFunction(s.jsonpCallback)?s.jsonpCallback():s.jsonpCallback;overwritten=window[callbackName];if(replaceInUrl){s.url=url.replace(rjsonp,"$1"+callbackName)}else if(replaceInData){s.data=data.replace(rjsonp,"$1"+callbackName)}else if(hasCallback){s.url+=(rquestion.test(url)?"&":"?")+s.jsonp+"="+callbackName}s.converters["script json"]=function(){if(!responseContainer){jQuery.error(callbackName+" was not called")}return responseContainer[0]};s.dataTypes[0]="json";window[callbackName]=function(){responseContainer=arguments};jqXHR.always(function(){window[callbackName]=overwritten;if(s[callbackName]){s.jsonpCallback=originalSettings.jsonpCallback;oldCallbacks.push(callbackName)}if(responseContainer&&jQuery.isFunction(overwritten)){overwritten(responseContainer[0])}responseContainer=overwritten=undefined});return"script"}});jQuery.ajaxSetup({accepts:{script:"text/javascript, application/javascript, application/ecmascript, application/x-ecmascript"},contents:{script:/javascript|ecmascript/},converters:{"text script":function(text){jQuery.globalEval(text);return text}}});jQuery.ajaxPrefilter("script",function(s){if(s.cache===undefined){s.cache=false}if(s.crossDomain){s.type="GET";s.global=false}});jQuery.ajaxTransport("script",function(s){if(s.crossDomain){var script,head=document.head||document.getElementsByTagName("head")[0]||document.documentElement;return{send:function(_,callback){script=document.createElement("script");script.async="async";if(s.scriptCharset){script.charset=s.scriptCharset}script.src=s.url;script.onload=script.onreadystatechange=function(_,isAbort){if(isAbort||!script.readyState||/loaded|complete/.test(script.readyState)){script.onload=script.onreadystatechange=null;if(head&&script.parentNode){head.removeChild(script)}script=undefined;if(!isAbort){callback(200,"success")}}};head.insertBefore(script,head.firstChild)},abort:function(){if(script){script.onload(0,1)}}}}});var xhrCallbacks,xhrOnUnloadAbort=window.ActiveXObject?function(){for(var key in xhrCallbacks){xhrCallbacks[key](0,1)}}:false,xhrId=0;function createStandardXHR(){try{return new window.XMLHttpRequest}catch(e){}}function createActiveXHR(){try{return new window.ActiveXObject("Microsoft.XMLHTTP")}catch(e){}}jQuery.ajaxSettings.xhr=window.ActiveXObject?function(){return!this.isLocal&&createStandardXHR()||createActiveXHR()}:createStandardXHR;(function(xhr){jQuery.extend(jQuery.support,{ajax:!!xhr,cors:!!xhr&&"withCredentials"in xhr})})(jQuery.ajaxSettings.xhr());if(jQuery.support.ajax){jQuery.ajaxTransport(function(s){if(!s.crossDomain||jQuery.support.cors){var callback;return{send:function(headers,complete){var handle,i,xhr=s.xhr();if(s.username){xhr.open(s.type,s.url,s.async,s.username,s.password)}else{xhr.open(s.type,s.url,s.async)}if(s.xhrFields){for(i in s.xhrFields){xhr[i]=s.xhrFields[i]}}if(s.mimeType&&xhr.overrideMimeType){xhr.overrideMimeType(s.mimeType)}if(!s.crossDomain&&!headers["X-Requested-With"]){headers["X-Requested-With"]="XMLHttpRequest"}try{for(i in headers){xhr.setRequestHeader(i,headers[i])}}catch(_){}xhr.send(s.hasContent&&s.data||null);callback=function(_,isAbort){var status,statusText,responseHeaders,responses,xml;try{if(callback&&(isAbort||xhr.readyState===4)){callback=undefined;if(handle){xhr.onreadystatechange=jQuery.noop;if(xhrOnUnloadAbort){delete xhrCallbacks[handle]}}if(isAbort){if(xhr.readyState!==4){xhr.abort()}}else{status=xhr.status;responseHeaders=xhr.getAllResponseHeaders();responses={};xml=xhr.responseXML;if(xml&&xml.documentElement){responses.xml=xml}try{responses.text=xhr.responseText}catch(_){}try{statusText=xhr.statusText}catch(e){statusText=""}if(!status&&s.isLocal&&!s.crossDomain){status=responses.text?200:404}else if(status===1223){status=204}}}}catch(firefoxAccessException){if(!isAbort){complete(-1,firefoxAccessException)}}if(responses){complete(status,statusText,responses,responseHeaders)}};if(!s.async){callback()}else if(xhr.readyState===4){setTimeout(callback,0)}else{handle=++xhrId;if(xhrOnUnloadAbort){if(!xhrCallbacks){xhrCallbacks={};jQuery(window).unload(xhrOnUnloadAbort)}xhrCallbacks[handle]=callback}xhr.onreadystatechange=callback}},abort:function(){if(callback){callback(0,1)}}}}})}var fxNow,timerId,rfxtypes=/^(?:toggle|show|hide)$/,rfxnum=new RegExp("^(?:([-+])=|)("+core_pnum+")([a-z%]*)$","i"),rrun=/queueHooks$/,animationPrefilters=[defaultPrefilter],tweeners={"*":[function(prop,value){var end,unit,tween=this.createTween(prop,value),parts=rfxnum.exec(value),target=tween.cur(),start=+target||0,scale=1,maxIterations=20;if(parts){end=+parts[2];unit=parts[3]||(jQuery.cssNumber[prop]?"":"px");if(unit!=="px"&&start){start=jQuery.css(tween.elem,prop,true)||end||1;do{scale=scale||".5";start=start/scale;jQuery.style(tween.elem,prop,start+unit)}while(scale!==(scale=tween.cur()/target)&&scale!==1&&--maxIterations)}tween.unit=unit;tween.start=start;tween.end=parts[1]?start+(parts[1]+1)*end:end}return tween}]};function createFxNow(){setTimeout(function(){fxNow=undefined},0);return fxNow=jQuery.now()}function createTweens(animation,props){jQuery.each(props,function(prop,value){var collection=(tweeners[prop]||[]).concat(tweeners["*"]),index=0,length=collection.length;for(;index<length;index++){if(collection[index].call(animation,prop,value)){return}}})}function Animation(elem,properties,options){var result,index=0,tweenerIndex=0,length=animationPrefilters.length,deferred=jQuery.Deferred().always(function(){delete tick.elem}),tick=function(){var currentTime=fxNow||createFxNow(),remaining=Math.max(0,animation.startTime+animation.duration-currentTime),percent=1-(remaining/animation.duration||0),index=0,length=animation.tweens.length;for(;index<length;index++){animation.tweens[index].run(percent)}deferred.notifyWith(elem,[animation,percent,remaining]);if(percent<1&&length){return remaining}else{deferred.resolveWith(elem,[animation]);return false}},animation=deferred.promise({elem:elem,props:jQuery.extend({},properties),opts:jQuery.extend(true,{specialEasing:{}},options),originalProperties:properties,originalOptions:options,startTime:fxNow||createFxNow(),duration:options.duration,tweens:[],createTween:function(prop,end,easing){var tween=jQuery.Tween(elem,animation.opts,prop,end,animation.opts.specialEasing[prop]||animation.opts.easing);animation.tweens.push(tween);return tween},stop:function(gotoEnd){var index=0,length=gotoEnd?animation.tweens.length:0;for(;index<length;index++){animation.tweens[index].run(1)}if(gotoEnd){deferred.resolveWith(elem,[animation,gotoEnd])}else{deferred.rejectWith(elem,[animation,gotoEnd])}return this}}),props=animation.props;propFilter(props,animation.opts.specialEasing);for(;index<length;index++){result=animationPrefilters[index].call(animation,elem,props,animation.opts);if(result){return result}}createTweens(animation,props);if(jQuery.isFunction(animation.opts.start)){animation.opts.start.call(elem,animation)}jQuery.fx.timer(jQuery.extend(tick,{anim:animation,queue:animation.opts.queue,elem:elem}));return animation.progress(animation.opts.progress).done(animation.opts.done,animation.opts.complete).fail(animation.opts.fail).always(animation.opts.always)}function propFilter(props,specialEasing){var index,name,easing,value,hooks;for(index in props){name=jQuery.camelCase(index);easing=specialEasing[name];value=props[index];if(jQuery.isArray(value)){easing=value[1];value=props[index]=value[0]}if(index!==name){props[name]=value;delete props[index]}hooks=jQuery.cssHooks[name];if(hooks&&"expand"in hooks){value=hooks.expand(value);delete props[name];for(index in value){if(!(index in props)){props[index]=value[index];specialEasing[index]=easing}}}else{specialEasing[name]=easing}}}jQuery.Animation=jQuery.extend(Animation,{tweener:function(props,callback){if(jQuery.isFunction(props)){callback=props;props=["*"]}else{props=props.split(" ")}var prop,index=0,length=props.length;for(;index<length;index++){prop=props[index];tweeners[prop]=tweeners[prop]||[];tweeners[prop].unshift(callback)}},prefilter:function(callback,prepend){if(prepend){animationPrefilters.unshift(callback)}else{animationPrefilters.push(callback)}}});function defaultPrefilter(elem,props,opts){var index,prop,value,length,dataShow,tween,hooks,oldfire,anim=this,style=elem.style,orig={},handled=[],hidden=elem.nodeType&&isHidden(elem);if(!opts.queue){hooks=jQuery._queueHooks(elem,"fx");if(hooks.unqueued==null){hooks.unqueued=0;oldfire=hooks.empty.fire;hooks.empty.fire=function(){if(!hooks.unqueued){oldfire()}}}hooks.unqueued++;anim.always(function(){anim.always(function(){hooks.unqueued--;if(!jQuery.queue(elem,"fx").length){hooks.empty.fire()}})})}if(elem.nodeType===1&&("height"in props||"width"in props)){opts.overflow=[style.overflow,style.overflowX,style.overflowY];if(jQuery.css(elem,"display")==="inline"&&jQuery.css(elem,"float")==="none"){if(!jQuery.support.inlineBlockNeedsLayout||css_defaultDisplay(elem.nodeName)==="inline"){style.display="inline-block"}else{style.zoom=1}}}if(opts.overflow){style.overflow="hidden";if(!jQuery.support.shrinkWrapBlocks){anim.done(function(){style.overflow=opts.overflow[0];style.overflowX=opts.overflow[1];style.overflowY=opts.overflow[2]})}}for(index in props){value=props[index];if(rfxtypes.exec(value)){delete props[index];if(value===(hidden?"hide":"show")){continue}handled.push(index)}}length=handled.length;if(length){dataShow=jQuery._data(elem,"fxshow")||jQuery._data(elem,"fxshow",{});if(hidden){jQuery(elem).show()}else{anim.done(function(){jQuery(elem).hide()})}anim.done(function(){var prop;jQuery.removeData(elem,"fxshow",true);for(prop in orig){jQuery.style(elem,prop,orig[prop])}});for(index=0;index<length;index++){prop=handled[index];tween=anim.createTween(prop,hidden?dataShow[prop]:0);orig[prop]=dataShow[prop]||jQuery.style(elem,prop);if(!(prop in dataShow)){dataShow[prop]=tween.start;if(hidden){tween.end=tween.start;tween.start=prop==="width"||prop==="height"?1:0}}}}}function Tween(elem,options,prop,end,easing){return new Tween.prototype.init(elem,options,prop,end,easing)}jQuery.Tween=Tween;Tween.prototype={constructor:Tween,init:function(elem,options,prop,end,easing,unit){this.elem=elem;this.prop=prop;this.easing=easing||"swing";this.options=options;this.start=this.now=this.cur();this.end=end;this.unit=unit||(jQuery.cssNumber[prop]?"":"px")},cur:function(){var hooks=Tween.propHooks[this.prop];return hooks&&hooks.get?hooks.get(this):Tween.propHooks._default.get(this)},run:function(percent){var eased,hooks=Tween.propHooks[this.prop];if(this.options.duration){this.pos=eased=jQuery.easing[this.easing](percent,this.options.duration*percent,0,1,this.options.duration)}else{this.pos=eased=percent}this.now=(this.end-this.start)*eased+this.start;if(this.options.step){this.options.step.call(this.elem,this.now,this)}if(hooks&&hooks.set){hooks.set(this)}else{Tween.propHooks._default.set(this)}return this}};Tween.prototype.init.prototype=Tween.prototype;Tween.propHooks={_default:{get:function(tween){var result;if(tween.elem[tween.prop]!=null&&(!tween.elem.style||tween.elem.style[tween.prop]==null)){return tween.elem[tween.prop]}result=jQuery.css(tween.elem,tween.prop,false,"");return!result||result==="auto"?0:result},set:function(tween){if(jQuery.fx.step[tween.prop]){jQuery.fx.step[tween.prop](tween)}else if(tween.elem.style&&(tween.elem.style[jQuery.cssProps[tween.prop]]!=null||jQuery.cssHooks[tween.prop])){jQuery.style(tween.elem,tween.prop,tween.now+tween.unit)}else{tween.elem[tween.prop]=tween.now}}}};Tween.propHooks.scrollTop=Tween.propHooks.scrollLeft={set:function(tween){if(tween.elem.nodeType&&tween.elem.parentNode){tween.elem[tween.prop]=tween.now}}};jQuery.each(["toggle","show","hide"],function(i,name){var cssFn=jQuery.fn[name];jQuery.fn[name]=function(speed,easing,callback){return speed==null||typeof speed==="boolean"||!i&&jQuery.isFunction(speed)&&jQuery.isFunction(easing)?cssFn.apply(this,arguments):this.animate(genFx(name,true),speed,easing,callback)}});jQuery.fn.extend({fadeTo:function(speed,to,easing,callback){return this.filter(isHidden).css("opacity",0).show().end().animate({opacity:to},speed,easing,callback)},animate:function(prop,speed,easing,callback){var empty=jQuery.isEmptyObject(prop),optall=jQuery.speed(speed,easing,callback),doAnimation=function(){var anim=Animation(this,jQuery.extend({},prop),optall);if(empty){anim.stop(true)}};return empty||optall.queue===false?this.each(doAnimation):this.queue(optall.queue,doAnimation)},stop:function(type,clearQueue,gotoEnd){var stopQueue=function(hooks){var stop=hooks.stop;delete hooks.stop;stop(gotoEnd)};if(typeof type!=="string"){gotoEnd=clearQueue;clearQueue=type;type=undefined}if(clearQueue&&type!==false){this.queue(type||"fx",[])}return this.each(function(){var dequeue=true,index=type!=null&&type+"queueHooks",timers=jQuery.timers,data=jQuery._data(this);if(index){if(data[index]&&data[index].stop){stopQueue(data[index])}}else{for(index in data){if(data[index]&&data[index].stop&&rrun.test(index)){stopQueue(data[index])}}}for(index=timers.length;index--;){if(timers[index].elem===this&&(type==null||timers[index].queue===type)){timers[index].anim.stop(gotoEnd);dequeue=false;timers.splice(index,1)}}if(dequeue||!gotoEnd){jQuery.dequeue(this,type)}})}});function genFx(type,includeWidth){var which,attrs={height:type},i=0;includeWidth=includeWidth?1:0;for(;i<4;i+=2-includeWidth){which=cssExpand[i];attrs["margin"+which]=attrs["padding"+which]=type}if(includeWidth){attrs.opacity=attrs.width=type}return attrs}jQuery.each({slideDown:genFx("show"),slideUp:genFx("hide"),slideToggle:genFx("toggle"),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"},fadeToggle:{opacity:"toggle"}},function(name,props){jQuery.fn[name]=function(speed,easing,callback){return this.animate(props,speed,easing,callback)
}});jQuery.speed=function(speed,easing,fn){var opt=speed&&typeof speed==="object"?jQuery.extend({},speed):{complete:fn||!fn&&easing||jQuery.isFunction(speed)&&speed,duration:speed,easing:fn&&easing||easing&&!jQuery.isFunction(easing)&&easing};opt.duration=jQuery.fx.off?0:typeof opt.duration==="number"?opt.duration:opt.duration in jQuery.fx.speeds?jQuery.fx.speeds[opt.duration]:jQuery.fx.speeds._default;if(opt.queue==null||opt.queue===true){opt.queue="fx"}opt.old=opt.complete;opt.complete=function(){if(jQuery.isFunction(opt.old)){opt.old.call(this)}if(opt.queue){jQuery.dequeue(this,opt.queue)}};return opt};jQuery.easing={linear:function(p){return p},swing:function(p){return.5-Math.cos(p*Math.PI)/2}};jQuery.timers=[];jQuery.fx=Tween.prototype.init;jQuery.fx.tick=function(){var timer,timers=jQuery.timers,i=0;for(;i<timers.length;i++){timer=timers[i];if(!timer()&&timers[i]===timer){timers.splice(i--,1)}}if(!timers.length){jQuery.fx.stop()}};jQuery.fx.timer=function(timer){if(timer()&&jQuery.timers.push(timer)&&!timerId){timerId=setInterval(jQuery.fx.tick,jQuery.fx.interval)}};jQuery.fx.interval=13;jQuery.fx.stop=function(){clearInterval(timerId);timerId=null};jQuery.fx.speeds={slow:600,fast:200,_default:400};jQuery.fx.step={};if(jQuery.expr&&jQuery.expr.filters){jQuery.expr.filters.animated=function(elem){return jQuery.grep(jQuery.timers,function(fn){return elem===fn.elem}).length}}var rroot=/^(?:body|html)$/i;jQuery.fn.offset=function(options){if(arguments.length){return options===undefined?this:this.each(function(i){jQuery.offset.setOffset(this,options,i)})}var docElem,body,win,clientTop,clientLeft,scrollTop,scrollLeft,box={top:0,left:0},elem=this[0],doc=elem&&elem.ownerDocument;if(!doc){return}if((body=doc.body)===elem){return jQuery.offset.bodyOffset(elem)}docElem=doc.documentElement;if(!jQuery.contains(docElem,elem)){return box}if(typeof elem.getBoundingClientRect!=="undefined"){box=elem.getBoundingClientRect()}win=getWindow(doc);clientTop=docElem.clientTop||body.clientTop||0;clientLeft=docElem.clientLeft||body.clientLeft||0;scrollTop=win.pageYOffset||docElem.scrollTop;scrollLeft=win.pageXOffset||docElem.scrollLeft;return{top:box.top+scrollTop-clientTop,left:box.left+scrollLeft-clientLeft}};jQuery.offset={bodyOffset:function(body){var top=body.offsetTop,left=body.offsetLeft;if(jQuery.support.doesNotIncludeMarginInBodyOffset){top+=parseFloat(jQuery.css(body,"marginTop"))||0;left+=parseFloat(jQuery.css(body,"marginLeft"))||0}return{top:top,left:left}},setOffset:function(elem,options,i){var position=jQuery.css(elem,"position");if(position==="static"){elem.style.position="relative"}var curElem=jQuery(elem),curOffset=curElem.offset(),curCSSTop=jQuery.css(elem,"top"),curCSSLeft=jQuery.css(elem,"left"),calculatePosition=(position==="absolute"||position==="fixed")&&jQuery.inArray("auto",[curCSSTop,curCSSLeft])>-1,props={},curPosition={},curTop,curLeft;if(calculatePosition){curPosition=curElem.position();curTop=curPosition.top;curLeft=curPosition.left}else{curTop=parseFloat(curCSSTop)||0;curLeft=parseFloat(curCSSLeft)||0}if(jQuery.isFunction(options)){options=options.call(elem,i,curOffset)}if(options.top!=null){props.top=options.top-curOffset.top+curTop}if(options.left!=null){props.left=options.left-curOffset.left+curLeft}if("using"in options){options.using.call(elem,props)}else{curElem.css(props)}}};jQuery.fn.extend({position:function(){if(!this[0]){return}var elem=this[0],offsetParent=this.offsetParent(),offset=this.offset(),parentOffset=rroot.test(offsetParent[0].nodeName)?{top:0,left:0}:offsetParent.offset();offset.top-=parseFloat(jQuery.css(elem,"marginTop"))||0;offset.left-=parseFloat(jQuery.css(elem,"marginLeft"))||0;parentOffset.top+=parseFloat(jQuery.css(offsetParent[0],"borderTopWidth"))||0;parentOffset.left+=parseFloat(jQuery.css(offsetParent[0],"borderLeftWidth"))||0;return{top:offset.top-parentOffset.top,left:offset.left-parentOffset.left}},offsetParent:function(){return this.map(function(){var offsetParent=this.offsetParent||document.body;while(offsetParent&&(!rroot.test(offsetParent.nodeName)&&jQuery.css(offsetParent,"position")==="static")){offsetParent=offsetParent.offsetParent}return offsetParent||document.body})}});jQuery.each({scrollLeft:"pageXOffset",scrollTop:"pageYOffset"},function(method,prop){var top=/Y/.test(prop);jQuery.fn[method]=function(val){return jQuery.access(this,function(elem,method,val){var win=getWindow(elem);if(val===undefined){return win?prop in win?win[prop]:win.document.documentElement[method]:elem[method]}if(win){win.scrollTo(!top?val:jQuery(win).scrollLeft(),top?val:jQuery(win).scrollTop())}else{elem[method]=val}},method,val,arguments.length,null)}});function getWindow(elem){return jQuery.isWindow(elem)?elem:elem.nodeType===9?elem.defaultView||elem.parentWindow:false}jQuery.each({Height:"height",Width:"width"},function(name,type){jQuery.each({padding:"inner"+name,content:type,"":"outer"+name},function(defaultExtra,funcName){jQuery.fn[funcName]=function(margin,value){var chainable=arguments.length&&(defaultExtra||typeof margin!=="boolean"),extra=defaultExtra||(margin===true||value===true?"margin":"border");return jQuery.access(this,function(elem,type,value){var doc;if(jQuery.isWindow(elem)){return elem.document.documentElement["client"+name]}if(elem.nodeType===9){doc=elem.documentElement;return Math.max(elem.body["scroll"+name],doc["scroll"+name],elem.body["offset"+name],doc["offset"+name],doc["client"+name])}return value===undefined?jQuery.css(elem,type,value,extra):jQuery.style(elem,type,value,extra)},type,chainable?margin:undefined,chainable,null)}})});window.jQuery=window.$=jQuery;if(typeof define==="function"&&define.amd&&define.amd.jQuery){define("jquery",[],function(){return jQuery})}})(window);window.Modernizr=function(a,b,c){function D(a){j.cssText=a}function E(a,b){return D(n.join(a+";")+(b||""))}function F(a,b){return typeof a===b}function G(a,b){return!!~(""+a).indexOf(b)}function H(a,b){for(var d in a){var e=a[d];if(!G(e,"-")&&j[e]!==c)return b=="pfx"?e:!0}return!1}function I(a,b,d){for(var e in a){var f=b[a[e]];if(f!==c)return d===!1?a[e]:F(f,"function")?f.bind(d||b):f}return!1}function J(a,b,c){var d=a.charAt(0).toUpperCase()+a.slice(1),e=(a+" "+p.join(d+" ")+d).split(" ");return F(b,"string")||F(b,"undefined")?H(e,b):(e=(a+" "+q.join(d+" ")+d).split(" "),I(e,b,c))}function K(){e.input=function(c){for(var d=0,e=c.length;d<e;d++)u[c[d]]=c[d]in k;return u.list&&(u.list=!!b.createElement("datalist")&&!!a.HTMLDataListElement),u}("autocomplete autofocus list placeholder max min multiple pattern required step".split(" ")),e.inputtypes=function(a){for(var d=0,e,f,h,i=a.length;d<i;d++)k.setAttribute("type",f=a[d]),e=k.type!=="text",e&&(k.value=l,k.style.cssText="position:absolute;visibility:hidden;",/^range$/.test(f)&&k.style.WebkitAppearance!==c?(g.appendChild(k),h=b.defaultView,e=h.getComputedStyle&&h.getComputedStyle(k,null).WebkitAppearance!=="textfield"&&k.offsetHeight!==0,g.removeChild(k)):/^(search|tel)$/.test(f)||(/^(url|email)$/.test(f)?e=k.checkValidity&&k.checkValidity()===!1:e=k.value!=l)),t[a[d]]=!!e;return t}("search tel url email datetime date month week time datetime-local number range color".split(" "))}var d="2.6.1",e={},f=!0,g=b.documentElement,h="modernizr",i=b.createElement(h),j=i.style,k=b.createElement("input"),l=":)",m={}.toString,n=" -webkit- -moz- -o- -ms- ".split(" "),o="Webkit Moz O ms",p=o.split(" "),q=o.toLowerCase().split(" "),r={svg:"http://www.w3.org/2000/svg"},s={},t={},u={},v=[],w=v.slice,x,y=function(a,c,d,e){var f,i,j,k=b.createElement("div"),l=b.body,m=l?l:b.createElement("body");if(parseInt(d,10))while(d--)j=b.createElement("div"),j.id=e?e[d]:h+(d+1),k.appendChild(j);return f=["&#173;",'<style id="s',h,'">',a,"</style>"].join(""),k.id=h,(l?k:m).innerHTML+=f,m.appendChild(k),l||(m.style.background="",g.appendChild(m)),i=c(k,a),l?k.parentNode.removeChild(k):m.parentNode.removeChild(m),!!i},z=function(b){var c=a.matchMedia||a.msMatchMedia;if(c)return c(b).matches;var d;return y("@media "+b+" { #"+h+" { position: absolute; } }",function(b){d=(a.getComputedStyle?getComputedStyle(b,null):b.currentStyle)["position"]=="absolute"}),d},A=function(){function d(d,e){e=e||b.createElement(a[d]||"div"),d="on"+d;var f=d in e;return f||(e.setAttribute||(e=b.createElement("div")),e.setAttribute&&e.removeAttribute&&(e.setAttribute(d,""),f=F(e[d],"function"),F(e[d],"undefined")||(e[d]=c),e.removeAttribute(d))),e=null,f}var a={select:"input",change:"input",submit:"form",reset:"form",error:"img",load:"img",abort:"img"};return d}(),B={}.hasOwnProperty,C;!F(B,"undefined")&&!F(B.call,"undefined")?C=function(a,b){return B.call(a,b)}:C=function(a,b){return b in a&&F(a.constructor.prototype[b],"undefined")},Function.prototype.bind||(Function.prototype.bind=function(b){var c=this;if(typeof c!="function")throw new TypeError;var d=w.call(arguments,1),e=function(){if(this instanceof e){var a=function(){};a.prototype=c.prototype;var f=new a,g=c.apply(f,d.concat(w.call(arguments)));return Object(g)===g?g:f}return c.apply(b,d.concat(w.call(arguments)))};return e}),s.flexbox=function(){return J("flexWrap")},s.canvas=function(){var a=b.createElement("canvas");return!!a.getContext&&!!a.getContext("2d")},s.canvastext=function(){return!!e.canvas&&!!F(b.createElement("canvas").getContext("2d").fillText,"function")},s.webgl=function(){return!!a.WebGLRenderingContext},s.touch=function(){var c;return"ontouchstart"in a||a.DocumentTouch&&b instanceof DocumentTouch?c=!0:y(["@media (",n.join("touch-enabled),("),h,")","{#modernizr{top:9px;position:absolute}}"].join(""),function(a){c=a.offsetTop===9}),c},s.geolocation=function(){return"geolocation"in navigator},s.postmessage=function(){return!!a.postMessage},s.websqldatabase=function(){return!!a.openDatabase},s.indexedDB=function(){return!!J("indexedDB",a)},s.hashchange=function(){return A("hashchange",a)&&(b.documentMode===c||b.documentMode>7)},s.history=function(){return!!a.history&&!!history.pushState},s.draganddrop=function(){var a=b.createElement("div");return"draggable"in a||"ondragstart"in a&&"ondrop"in a},s.websockets=function(){return"WebSocket"in a||"MozWebSocket"in a},s.rgba=function(){return D("background-color:rgba(150,255,150,.5)"),G(j.backgroundColor,"rgba")},s.hsla=function(){return D("background-color:hsla(120,40%,100%,.5)"),G(j.backgroundColor,"rgba")||G(j.backgroundColor,"hsla")},s.multiplebgs=function(){return D("background:url(https://),url(https://),red url(https://)"),/(url\s*\(.*?){3}/.test(j.background)},s.backgroundsize=function(){return J("backgroundSize")},s.borderimage=function(){return J("borderImage")},s.borderradius=function(){return J("borderRadius")},s.boxshadow=function(){return J("boxShadow")},s.textshadow=function(){return b.createElement("div").style.textShadow===""},s.opacity=function(){return E("opacity:.55"),/^0.55$/.test(j.opacity)},s.cssanimations=function(){return J("animationName")},s.csscolumns=function(){return J("columnCount")},s.cssgradients=function(){var a="background-image:",b="gradient(linear,left top,right bottom,from(#9f9),to(white));",c="linear-gradient(left top,#9f9, white);";return D((a+"-webkit- ".split(" ").join(b+a)+n.join(c+a)).slice(0,-a.length)),G(j.backgroundImage,"gradient")},s.cssreflections=function(){return J("boxReflect")},s.csstransforms=function(){return!!J("transform")},s.csstransforms3d=function(){var a=!!J("perspective");return a&&"webkitPerspective"in g.style&&y("@media (transform-3d),(-webkit-transform-3d){#modernizr{left:9px;position:absolute;height:3px;}}",function(b,c){a=b.offsetLeft===9&&b.offsetHeight===3}),a},s.csstransitions=function(){return J("transition")},s.fontface=function(){var a;return y('@font-face {font-family:"font";src:url("https://")}',function(c,d){var e=b.getElementById("smodernizr"),f=e.sheet||e.styleSheet,g=f?f.cssRules&&f.cssRules[0]?f.cssRules[0].cssText:f.cssText||"":"";a=/src/i.test(g)&&g.indexOf(d.split(" ")[0])===0}),a},s.generatedcontent=function(){var a;return y(['#modernizr:after{content:"',l,'";visibility:hidden}'].join(""),function(b){a=b.offsetHeight>=1}),a},s.video=function(){var a=b.createElement("video"),c=!1;try{if(c=!!a.canPlayType)c=new Boolean(c),c.ogg=a.canPlayType('video/ogg; codecs="theora"').replace(/^no$/,""),c.h264=a.canPlayType('video/mp4; codecs="avc1.42E01E"').replace(/^no$/,""),c.webm=a.canPlayType('video/webm; codecs="vp8, vorbis"').replace(/^no$/,"")}catch(d){}return c},s.audio=function(){var a=b.createElement("audio"),c=!1;try{if(c=!!a.canPlayType)c=new Boolean(c),c.ogg=a.canPlayType('audio/ogg; codecs="vorbis"').replace(/^no$/,""),c.mp3=a.canPlayType("audio/mpeg;").replace(/^no$/,""),c.wav=a.canPlayType('audio/wav; codecs="1"').replace(/^no$/,""),c.m4a=(a.canPlayType("audio/x-m4a;")||a.canPlayType("audio/aac;")).replace(/^no$/,"")}catch(d){}return c},s.localstorage=function(){try{return localStorage.setItem(h,h),localStorage.removeItem(h),!0}catch(a){return!1}},s.sessionstorage=function(){try{return sessionStorage.setItem(h,h),sessionStorage.removeItem(h),!0}catch(a){return!1}},s.webworkers=function(){return!!a.Worker},s.applicationcache=function(){return!!a.applicationCache},s.svg=function(){return!!b.createElementNS&&!!b.createElementNS(r.svg,"svg").createSVGRect},s.inlinesvg=function(){var a=b.createElement("div");return a.innerHTML="<svg/>",(a.firstChild&&a.firstChild.namespaceURI)==r.svg},s.smil=function(){return!!b.createElementNS&&/SVGAnimate/.test(m.call(b.createElementNS(r.svg,"animate")))},s.svgclippaths=function(){return!!b.createElementNS&&/SVGClipPath/.test(m.call(b.createElementNS(r.svg,"clipPath")))};for(var L in s)C(s,L)&&(x=L.toLowerCase(),e[x]=s[L](),v.push((e[x]?"":"no-")+x));return e.input||K(),e.addTest=function(a,b){if(typeof a=="object")for(var d in a)C(a,d)&&e.addTest(d,a[d]);else{a=a.toLowerCase();if(e[a]!==c)return e;b=typeof b=="function"?b():b,f&&(g.className+=" "+(b?"":"no-")+a),e[a]=b}return e},D(""),i=k=null,function(a,b){function k(a,b){var c=a.createElement("p"),d=a.getElementsByTagName("head")[0]||a.documentElement;return c.innerHTML="x<style>"+b+"</style>",d.insertBefore(c.lastChild,d.firstChild)}function l(){var a=r.elements;return typeof a=="string"?a.split(" "):a}function m(a){var b=i[a[g]];return b||(b={},h++,a[g]=h,i[h]=b),b}function n(a,c,f){c||(c=b);if(j)return c.createElement(a);f||(f=m(c));var g;return f.cache[a]?g=f.cache[a].cloneNode():e.test(a)?g=(f.cache[a]=f.createElem(a)).cloneNode():g=f.createElem(a),g.canHaveChildren&&!d.test(a)?f.frag.appendChild(g):g}function o(a,c){a||(a=b);if(j)return a.createDocumentFragment();c=c||m(a);var d=c.frag.cloneNode(),e=0,f=l(),g=f.length;for(;e<g;e++)d.createElement(f[e]);return d}function p(a,b){b.cache||(b.cache={},b.createElem=a.createElement,b.createFrag=a.createDocumentFragment,b.frag=b.createFrag()),a.createElement=function(c){return r.shivMethods?n(c,a,b):b.createElem(c)},a.createDocumentFragment=Function("h,f","return function(){var n=f.cloneNode(),c=n.createElement;h.shivMethods&&("+l().join().replace(/\w+/g,function(a){return b.createElem(a),b.frag.createElement(a),'c("'+a+'")'})+");return n}")(r,b.frag)}function q(a){a||(a=b);var c=m(a);return r.shivCSS&&!f&&!c.hasCSS&&(c.hasCSS=!!k(a,"article,aside,figcaption,figure,footer,header,hgroup,nav,section{display:block}mark{background:#FF0;color:#000}")),j||p(a,c),a}var c=a.html5||{},d=/^<|^(?:button|map|select|textarea|object|iframe|option|optgroup)$/i,e=/^<|^(?:a|b|button|code|div|fieldset|form|h1|h2|h3|h4|h5|h6|i|iframe|img|input|label|li|link|ol|option|p|param|q|script|select|span|strong|style|table|tbody|td|textarea|tfoot|th|thead|tr|ul)$/i,f,g="_html5shiv",h=0,i={},j;(function(){try{var a=b.createElement("a");a.innerHTML="<xyz></xyz>",f="hidden"in a,j=a.childNodes.length==1||function(){b.createElement("a");var a=b.createDocumentFragment();return typeof a.cloneNode=="undefined"||typeof a.createDocumentFragment=="undefined"||typeof a.createElement=="undefined"}()}catch(c){f=!0,j=!0}})();var r={elements:c.elements||"abbr article aside audio bdi canvas data datalist details figcaption figure footer header hgroup mark meter nav output progress section summary time video",shivCSS:c.shivCSS!==!1,supportsUnknownElements:j,shivMethods:c.shivMethods!==!1,type:"default",shivDocument:q,createElement:n,createDocumentFragment:o};a.html5=r,q(b)}(this,b),e._version=d,e._prefixes=n,e._domPrefixes=q,e._cssomPrefixes=p,e.mq=z,e.hasEvent=A,e.testProp=function(a){return H([a])},e.testAllProps=J,e.testStyles=y,e.prefixed=function(a,b,c){return b?J(a,b,c):J(a,"pfx")},g.className=g.className.replace(/(^|\s)no-js(\s|$)/,"$1$2")+(f?" js "+v.join(" "):""),e}(this,this.document),function(a,b,c){function d(a){return o.call(a)=="[object Function]"}function e(a){return typeof a=="string"}function f(){}function g(a){return!a||a=="loaded"||a=="complete"||a=="uninitialized"}function h(){var a=p.shift();q=1,a?a.t?m(function(){(a.t=="c"?B.injectCss:B.injectJs)(a.s,0,a.a,a.x,a.e,1)},0):(a(),h()):q=0}function i(a,c,d,e,f,i,j){function k(b){if(!o&&g(l.readyState)&&(u.r=o=1,!q&&h(),l.onload=l.onreadystatechange=null,b)){a!="img"&&m(function(){t.removeChild(l)},50);for(var d in y[c])y[c].hasOwnProperty(d)&&y[c][d].onload()}}var j=j||B.errorTimeout,l={},o=0,r=0,u={t:d,s:c,e:f,a:i,x:j};y[c]===1&&(r=1,y[c]=[],l=b.createElement(a)),a=="object"?l.data=c:(l.src=c,l.type=a),l.width=l.height="0",l.onerror=l.onload=l.onreadystatechange=function(){k.call(this,r)},p.splice(e,0,u),a!="img"&&(r||y[c]===2?(t.insertBefore(l,s?null:n),m(k,j)):y[c].push(l))}function j(a,b,c,d,f){return q=0,b=b||"j",e(a)?i(b=="c"?v:u,a,b,this.i++,c,d,f):(p.splice(this.i++,0,a),p.length==1&&h()),this}function k(){var a=B;return a.loader={load:j,i:0},a}var l=b.documentElement,m=a.setTimeout,n=b.getElementsByTagName("script")[0],o={}.toString,p=[],q=0,r="MozAppearance"in l.style,s=r&&!!b.createRange().compareNode,t=s?l:n.parentNode,l=a.opera&&o.call(a.opera)=="[object Opera]",l=!!b.attachEvent&&!l,u=r?"object":l?"script":"img",v=l?"script":u,w=Array.isArray||function(a){return o.call(a)=="[object Array]"},x=[],y={},z={timeout:function(a,b){return b.length&&(a.timeout=b[0]),a}},A,B;B=function(a){function b(a){var a=a.split("!"),b=x.length,c=a.pop(),d=a.length,c={url:c,origUrl:c,prefixes:a},e,f,g;for(f=0;f<d;f++)g=a[f].split("="),(e=z[g.shift()])&&(c=e(c,g));for(f=0;f<b;f++)c=x[f](c);return c}function g(a,e,f,g,i){var j=b(a),l=j.autoCallback;j.url.split(".").pop().split("?").shift(),j.bypass||(e&&(e=d(e)?e:e[a]||e[g]||e[a.split("/").pop().split("?")[0]]||h),j.instead?j.instead(a,e,f,g,i):(y[j.url]?j.noexec=!0:y[j.url]=1,f.load(j.url,j.forceCSS||!j.forceJS&&"css"==j.url.split(".").pop().split("?").shift()?"c":c,j.noexec,j.attrs,j.timeout),(d(e)||d(l))&&f.load(function(){k(),e&&e(j.origUrl,i,g),l&&l(j.origUrl,i,g),y[j.url]=2})))}function i(a,b){function c(a,c){if(a){if(e(a))c||(j=function(){var a=[].slice.call(arguments);k.apply(this,a),l()}),g(a,j,b,0,h);else if(Object(a)===a)for(n in m=function(){var b=0,c;for(c in a)a.hasOwnProperty(c)&&b++;return b}(),a)a.hasOwnProperty(n)&&(!c&&!--m&&(d(j)?j=function(){var a=[].slice.call(arguments);k.apply(this,a),l()}:j[n]=function(a){return function(){var b=[].slice.call(arguments);a&&a.apply(this,b),l()}}(k[n])),g(a[n],j,b,n,h))}else!c&&l()}var h=!!a.test,i=a.load||a.both,j=a.callback||f,k=j,l=a.complete||f,m,n;c(h?a.yep:a.nope,!!i),i&&c(i)}var j,l,m=this.yepnope.loader;if(e(a))g(a,0,m,0);else if(w(a))for(j=0;j<a.length;j++)l=a[j],e(l)?g(l,0,m,0):w(l)?B(l):Object(l)===l&&i(l,m);else Object(a)===a&&i(a,m)},B.addPrefix=function(a,b){z[a]=b},B.addFilter=function(a){x.push(a)},B.errorTimeout=1e4,b.readyState==null&&b.addEventListener&&(b.readyState="loading",b.addEventListener("DOMContentLoaded",A=function(){b.removeEventListener("DOMContentLoaded",A,0),b.readyState="complete"},0)),a.yepnope=k(),a.yepnope.executeStack=h,a.yepnope.injectJs=function(a,c,d,e,i,j){var k=b.createElement("script"),l,o,e=e||B.errorTimeout;k.src=a;for(o in d)k.setAttribute(o,d[o]);c=j?h:c||f,k.onreadystatechange=k.onload=function(){!l&&g(k.readyState)&&(l=1,c(),k.onload=k.onreadystatechange=null)},m(function(){l||(l=1,c(1))},e),i?k.onload():n.parentNode.insertBefore(k,n)},a.yepnope.injectCss=function(a,c,d,e,g,i){var e=b.createElement("link"),j,c=i?h:c||f;e.href=a,e.rel="stylesheet",e.type="text/css";for(j in d)e.setAttribute(j,d[j]);g||(n.parentNode.insertBefore(e,n),m(c,0))}}(this,document),Modernizr.load=function(){yepnope.apply(window,[].slice.call(arguments,0))};(function(root,factory){if(typeof define==="function"&&define.amd){define(["$","handlebars"],factory)}else{root.Ventus=factory(root.$,root.Handlebars)}})(this,function($,Handlebars){var requirejs,require,define;(function(undef){var defined={},waiting={},config={},defining={},aps=[].slice,main,req;function normalize(name,baseName){var baseParts=baseName&&baseName.split("/"),map=config.map,starMap=map&&map["*"]||{},nameParts,nameSegment,mapValue,foundMap,foundI,foundStarMap,starI,i,j,part;if(name&&name.charAt(0)==="."){if(baseName){baseParts=baseParts.slice(0,baseParts.length-1);name=baseParts.concat(name.split("/"));for(i=0;part=name[i];i++){if(part==="."){name.splice(i,1);i-=1}else if(part===".."){if(i===1&&(name[2]===".."||name[0]==="..")){return true}else if(i>0){name.splice(i-1,2);i-=2}}}name=name.join("/")}}if((baseParts||starMap)&&map){nameParts=name.split("/");for(i=nameParts.length;i>0;i-=1){nameSegment=nameParts.slice(0,i).join("/");if(baseParts){for(j=baseParts.length;j>0;j-=1){mapValue=map[baseParts.slice(0,j).join("/")];if(mapValue){mapValue=mapValue[nameSegment];if(mapValue){foundMap=mapValue;foundI=i;break}}}}if(foundMap){break}if(!foundStarMap&&starMap&&starMap[nameSegment]){foundStarMap=starMap[nameSegment];starI=i}}if(!foundMap&&foundStarMap){foundMap=foundStarMap;foundI=starI}if(foundMap){nameParts.splice(0,foundI,foundMap);name=nameParts.join("/")}}return name}function makeRequire(relName,forceSync){return function(){return req.apply(undef,aps.call(arguments,0).concat([relName,forceSync]))}}function makeNormalize(relName){return function(name){return normalize(name,relName)}}function makeLoad(depName){return function(value){defined[depName]=value}}function callDep(name){if(waiting.hasOwnProperty(name)){var args=waiting[name];delete waiting[name];defining[name]=true;main.apply(undef,args)}if(!defined.hasOwnProperty(name)){throw new Error("No "+name)}return defined[name]}function makeMap(name,relName){var prefix,plugin,index=name.indexOf("!");if(index!==-1){prefix=normalize(name.slice(0,index),relName);name=name.slice(index+1);plugin=callDep(prefix);if(plugin&&plugin.normalize){name=plugin.normalize(name,makeNormalize(relName))}else{name=normalize(name,relName)}}else{name=normalize(name,relName)}return{f:prefix?prefix+"!"+name:name,n:name,p:plugin}}function makeConfig(name){return function(){return config&&config.config&&config.config[name]||{}}}main=function(name,deps,callback,relName){var args=[],usingExports,cjsModule,depName,ret,map,i;relName=relName||name;if(typeof callback==="function"){deps=!deps.length&&callback.length?["require","exports","module"]:deps;for(i=0;i<deps.length;i++){map=makeMap(deps[i],relName);depName=map.f;if(depName==="require"){args[i]=makeRequire(name)}else if(depName==="exports"){args[i]=defined[name]={};usingExports=true}else if(depName==="module"){cjsModule=args[i]={id:name,uri:"",exports:defined[name],config:makeConfig(name)}}else if(defined.hasOwnProperty(depName)||waiting.hasOwnProperty(depName)){args[i]=callDep(depName)}else if(map.p){map.p.load(map.n,makeRequire(relName,true),makeLoad(depName),{});args[i]=defined[depName]}else if(!defining[depName]){throw new Error(name+" missing "+depName)}}ret=callback.apply(defined[name],args);if(name){if(cjsModule&&cjsModule.exports!==undef&&cjsModule.exports!==defined[name]){defined[name]=cjsModule.exports}else if(ret!==undef||!usingExports){defined[name]=ret}}}else if(name){defined[name]=callback}};requirejs=require=req=function(deps,callback,relName,forceSync){if(typeof deps==="string"){return callDep(makeMap(deps,callback).f)}else if(!deps.splice){config=deps;if(callback.splice){deps=callback;callback=relName;relName=null}else{deps=undef}}callback=callback||function(){};if(forceSync){main(undef,deps,callback,relName)}else{setTimeout(function(){main(undef,deps,callback,relName)},15)}return req};req.config=function(cfg){config=cfg;return req};define=function(name,deps,callback){if(!deps.splice){callback=deps;deps=[]}waiting[name]=[name,deps,callback]};define.amd={jQuery:true}})();define("almond",function(){});(function(root){function equals(slot,scope,expected){return function(item){return(item.funct===slot&&item.scope===scope)===expected}}function hasListener(listeners,signal,slot,scope){if(!listeners[signal])return false;return listeners[signal].some(equals(slot,scope,true))}function Emitter(){this._listeners={}}Emitter.prototype={listenersCount:function(signal){var list=this._listeners[signal];return list?list.length:0},on:function on(signal,slot,scope){var list=this._listeners;if(hasListener(list,signal,slot,scope))return;if(!list[signal])list[signal]=[];list[signal].push({funct:slot,scope:scope})},off:function off(signal,slot,scope){var list=this._listeners[signal];if(!list)return;this._listeners[signal]=list.filter(equals(slot,scope,false))},once:function once(signal,slot,scope){if(hasListener(this._listeners,signal,slot,scope))return;this.on(signal,function wrapper(){this.off(signal,wrapper,this);slot.apply(scope,arguments)},this)},emit:function emit(signal){var list=this._listeners[signal];if(!list)return;var data=Array.prototype.slice.call(arguments,1);list.forEach(function(item){item.funct.apply(item.scope,data)})},connect:function connect(slots,scope){if(!slots)return;for(var signal in slots){if(slots.hasOwnProperty(signal))this.on(signal,slots[signal],scope)}},disconnect:function disconnect(slots,scope){if(!slots)return;for(var signal in slots){if(slots.hasOwnProperty(signal))this.off(signal,slots[signal],scope)}}};if(typeof module!=="undefined"&&module.exports)module.exports=Emitter;else if(typeof define!=="undefined"&&define.amd)define("ventus/core/emitter",[],function(){return Emitter});else root.Emitter=Emitter})(this);define("ventus/core/view",["$"],function($,_){var splitter=/^(?:(.*)\s)?(\w+)$/;var transitionEventNames="transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd";var animationEventNames="animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd";var hooks=["transform","transition","animation","transform-origin"];for(var i=hooks.length;i--;){(function(property){$.cssHooks[property]={get:function(elem,computed,extra){return null},set:function(elem,value){elem.style["-webkit-"+property]=value;elem.style["-moz-"+property]=value;elem.style["-ms-"+property]=value;elem.style["-o-"+property]=value;elem.style[property]=value}}})(hooks[i])}$.fn.extend({listen:function(map,scope){var handler,data,selector,event;for(var key in map){if(!map.hasOwnProperty(key))continue;handler=map[key];data=key.match(splitter);selector=data[1];event=data[2];if(event==="mousedown")event+=" touchstart";else if(event==="mousemove")event+=" touchmove";else if(event==="mouseup")event+=" touchend";else if(event==="click")event+=" touchend";if(typeof handler==="string")handler=scope[handler];if(!handler)throw new Error("Handler not found");if(selector)this.on(event,selector,handler.bind(scope));else this.on(event,handler.bind(scope))}return this},onTransitionEnd:function(callback,scope){this.one(transitionEventNames,function(){callback.apply(scope||this)})},onAnimationEnd:function(callback,scope){this.one(animationEventNames,function(){callback.apply(scope||this)})}});return function(root){if(typeof root==="function"){return function(options){return $(root(options||{}))}}else{return $(root)}}});define("tpl",["require"],function(require){var extension=".tpl";function load(name,req,done,config){req(["handlebars"],function(Handlebars){var templateName=name.replace(/^.*[\\\/]/,"")+extension;if(config.isBuild){req([name+extension]);done();return}{req([name+extension],function(){done(Handlebars.templates[templateName])})}})}function write(pluginName,name,write){}return{load:load,write:write}});define("ventus/tpl/window.tpl",["handlebars"],function(Handlebars){var template=Handlebars.template,templates=Handlebars.templates=Handlebars.templates||{};templates["window.tpl"]=template(function(Handlebars,depth0,helpers,partials,data){helpers=helpers||Handlebars.helpers;var buffer="",stack1,foundHelper,functionType="function",escapeExpression=this.escapeExpression;buffer+='<div class="wm-window ';foundHelper=helpers.classname;if(foundHelper){stack1=foundHelper.call(depth0,{hash:{}})}else{stack1=depth0.classname;stack1=typeof stack1===functionType?stack1():stack1}buffer+=escapeExpression(stack1)+'" >\n	<div class="wm-window-box">\n		<header class="wm-window-title" unselectable="on">\n			<h1 unselectable="on">';foundHelper=helpers.title;if(foundHelper){stack1=foundHelper.call(depth0,{hash:{}})}else{stack1=depth0.title;stack1=typeof stack1===functionType?stack1():stack1}buffer+=escapeExpression(stack1)+'</h1>\n			<div class="wm-button-group">\n							<button class="wm-maximize">&nbsp;</button>\n				<button class="wm-close">&nbsp;</button>\n			</div>\n		</header>\n\n		<section class="wm-content"></section>\n\n		<button class="wm-resize">&nbsp;</button>\n	</div>\n	<div class="wm-window-overlay"></div>\n</div>\n';return buffer})});define("less",[],function(){var plugin={options:{},load:function load(name,parentRequire,done,config){var ext;this.options=config.css;if(config.isBuild||!config.debug){done();return}done()},write:function write(pluginName,name,write){}};return plugin});define("ventus/wm/window",["ventus/core/emitter","ventus/core/view","tpl!ventus/tpl/window","less!ventus/css/window"],function(Emitter,View,WindowTemplate){var Window=function(options){this.signals=new Emitter;options=options||{title:"Untitle Window",width:400,height:200,x:0,y:0,content:"",movable:true,resizable:true,widget:false,titlebar:true};this.el=View(WindowTemplate({title:options.title,classname:options.classname||""}));this.el.listen(this.events.window,this);if(options.opacity)this.el.css("opacity",options.opacity);this.$content=this.el.find(".wm-content");if(options.content)this.$content.append(options.content);this.$titlebar=this.el.find("header");this.width=options.width||400;this.height=options.height||200;this.x=options.x||0;this.y=options.y||0;this.z=1e4;this.opened=false;this.enabled=true;this.active=false;this.closed=false;this.maximized=false;this.minimized=false;this.widget=false;this.movable=true;this.resizable=typeof options.resizable!=="undefined"?options.resizable:true;this.titlebar=true};Window.prototype={_restore:null,_moving:null,_resizing:null,slots:{move:function(e){if(!this.enabled||!this.movable)return;var moveReference=e.originalEvent.targetTouches&&e.originalEvent.targetTouches.length==1?e.originalEvent.targetTouches[0]:e.originalEvent;this._moving=this.toLocal({x:moveReference.pageX,y:moveReference.pageY});this.el.addClass("move");e.preventDefault()}},events:{window:{click:function(e){this.signals.emit("select",this,e)},mousedown:function(e){this.focus();if(this.widget)this.slots.move.call(this,e)},".wm-content click":function(e){if(this.enabled)this.signals.emit("click",this,e)},".wm-window-title mousedown":function(e){this.slots.move.call(this,e)},".wm-window-title button.wm-close click":function(e){e.stopPropagation();e.preventDefault();if(this.enabled)this.close()},".wm-window-title button.wm-maximize click":function(e){e.stopPropagation();e.preventDefault();if(this.enabled&&this.resizable)this.maximize()},".wm-window-title button mousedown":function(e){this.focus();e.stopPropagation();e.preventDefault()},"button.wm-resize mousedown":function(e){if(!this.enabled||!this.resizable)return;var moveReference=e.originalEvent.targetTouches&&e.originalEvent.targetTouches.length==1?e.originalEvent.targetTouches[0]:e.originalEvent;this._resizing={width:this.width-moveReference.pageX,height:this.height-moveReference.pageY};
this.el.addClass("resizing");e.preventDefault()}},space:{mousemove:function(e){var moveReference=e.originalEvent.targetTouches&&e.originalEvent.targetTouches.length==1?e.originalEvent.targetTouches[0]:e.originalEvent;if(e.originalEvent.targetTouches&&e.originalEvent.targetTouches.length==1){moveReference=e.originalEvent.targetTouches[0]}else{moveReference=e.originalEvent}if(this._moving)this.move(moveReference.pageX-this._moving.x,moveReference.pageY-this._moving.y);if(this._resizing)this.resize(moveReference.pageX+this._resizing.width,moveReference.pageY+this._resizing.height)},mouseup:function(){if(this._moving){this.el.removeClass("move");this._moving=null}if(this._resizing){this.el.removeClass("resizing");this._restore=null;this._resizing=null}}}},set space(el){if(el&&!el.listen){console.error("The given space element is not a valid View");return}this._space=el;el.append(this.el);el.listen(this.events.space,this)},get space(){return this._space},get maximized(){return this._maximized},set maximized(value){if(value){this._restoreMaximized=this.stamp();this.signals.emit("maximize",this,this._restoreMaximized)}else{this.signals.emit("restore",this,this._restoreMaximized)}this._maximized=value},get minimized(){return this._minimized},set minimized(value){if(value){this._restoreMinimized=this.stamp();this.signals.emit("minimize",this,this._restoreMinimized)}else{this.signals.emit("restore",this,this._restoreMinimized)}this._minimized=value},set active(value){if(value){this.signals.emit("focus",this);this.el.addClass("active");this.el.removeClass("inactive")}else{this.signals.emit("blur",this);this.el.removeClass("active");this.el.addClass("inactive")}this._active=value},get active(){return this._active},set enabled(value){if(!value){this.el.addClass("disabled")}else{this.el.removeClass("disabled")}this._enabled=value},get enabled(){return this._enabled},set movable(value){this._movable=!!value},get movable(){return this._movable},set resizable(value){if(!value){this.el.addClass("noresizable")}else{this.el.removeClass("noresizable")}this._resizable=!!value},get resizable(){return this._resizable},set closed(value){if(value){this.signals.emit("close",this);this.el.addClass("closing");this.el.onAnimationEnd(function(){this.el.removeClass("closing");this.el.addClass("closed");this.el.hide();this.$content.html("")},this)}this._closed=value},get closed(){return this._closed},set opened(value){if(value){this.signals.emit("open",this);this.el.show();this.el.addClass("opening");this.el.onAnimationEnd(function(){this.el.removeClass("opening")},this)}this._opened=value},get opened(){return this._opened},set widget(value){this._widget=value},get widget(){return this._widget},set titlebar(value){if(value)this.$titlebar.removeClass("hide");else this.$titlebar.addClass("hide");this._titlebar=value},get titlebar(){return this._titlebar},set width(value){this.el.width(value)},get width(){return parseInt(this.el.width(),10)},set height(value){this.el.height(value)},get height(){return parseInt(this.el.height(),10)},set x(value){this.el.css("left",value)},set y(value){this.el.css("top",value)},get x(){return parseInt(this.el.css("left"),10)},get y(){return parseInt(this.el.css("top"),10)},set z(value){this.el.css("z-index",value)},get z(){return parseInt(this.el.css("z-index"),10)},open:function(){this.opened=true;return this},resize:function(w,h){this.width=w;this.height=h;return this},move:function(x,y){this.x=x;this.y=y;return this},stamp:function(){this.restore=function(){var size={width:this.width,height:this.height};var pos={x:this.x,y:this.y};return function(){this.resize(size.width,size.height);this.move(pos.x,pos.y);return this}}.apply(this);return this.restore},restore:function(){},maximize:function(){this.el.addClass("maximazing");this.el.onTransitionEnd(function(){this.el.removeClass("maximazing")},this);this.maximized=!this.maximized;return this},minimize:function(){this.el.addClass("minimizing");this.el.onTransitionEnd(function(){this.el.removeClass("minimizing")},this);this.minimized=!this.minimized;return this},close:function(){this.closed=true;return this},focus:function(){this.active=true;return this},blur:function(){this.active=false;return this},toLocal:function(coord){return{x:coord.x-this.x,y:coord.y-this.y}},toGlobal:function(coord){return{x:coord.x+this.x,y:coord.y+this.y}},append:function(el){el.appendTo(this.$content)}};return Window});define("ventus/wm/modes/default",["less!../../../css/windowmanager"],function(){var DefaultMode={register:function(){console.log("Default mode registered.")},plug:function(){},unplug:function(){},actions:{maximize:function(win){win.move(0,0);win.el.css("-webkit-transform","translate3d(0, 0, 0);");win.resize(this.el.width(),this.el.height())},restore:function(win,restore){restore.call(win)},minimize:function(win){win.resize(0,0)}}};return DefaultMode});(function(){var n=this,t=n._,r={},e=Array.prototype,u=Object.prototype,i=Function.prototype,a=e.push,o=e.slice,c=e.concat,l=u.toString,f=u.hasOwnProperty,s=e.forEach,p=e.map,h=e.reduce,v=e.reduceRight,d=e.filter,g=e.every,m=e.some,y=e.indexOf,b=e.lastIndexOf,x=Array.isArray,_=Object.keys,j=i.bind,w=function(n){return n instanceof w?n:this instanceof w?(this._wrapped=n,void 0):new w(n)};"undefined"!=typeof exports?("undefined"!=typeof module&&module.exports&&(exports=module.exports=w),exports._=w):n._=w,w.VERSION="1.4.4";var A=w.each=w.forEach=function(n,t,e){if(null!=n)if(s&&n.forEach===s)n.forEach(t,e);else if(n.length===+n.length){for(var u=0,i=n.length;i>u;u++)if(t.call(e,n[u],u,n)===r)return}else for(var a in n)if(w.has(n,a)&&t.call(e,n[a],a,n)===r)return};w.map=w.collect=function(n,t,r){var e=[];return null==n?e:p&&n.map===p?n.map(t,r):(A(n,function(n,u,i){e[e.length]=t.call(r,n,u,i)}),e)};var O="Reduce of empty array with no initial value";w.reduce=w.foldl=w.inject=function(n,t,r,e){var u=arguments.length>2;if(null==n&&(n=[]),h&&n.reduce===h)return e&&(t=w.bind(t,e)),u?n.reduce(t,r):n.reduce(t);if(A(n,function(n,i,a){u?r=t.call(e,r,n,i,a):(r=n,u=!0)}),!u)throw new TypeError(O);return r},w.reduceRight=w.foldr=function(n,t,r,e){var u=arguments.length>2;if(null==n&&(n=[]),v&&n.reduceRight===v)return e&&(t=w.bind(t,e)),u?n.reduceRight(t,r):n.reduceRight(t);var i=n.length;if(i!==+i){var a=w.keys(n);i=a.length}if(A(n,function(o,c,l){c=a?a[--i]:--i,u?r=t.call(e,r,n[c],c,l):(r=n[c],u=!0)}),!u)throw new TypeError(O);return r},w.find=w.detect=function(n,t,r){var e;return E(n,function(n,u,i){return t.call(r,n,u,i)?(e=n,!0):void 0}),e},w.filter=w.select=function(n,t,r){var e=[];return null==n?e:d&&n.filter===d?n.filter(t,r):(A(n,function(n,u,i){t.call(r,n,u,i)&&(e[e.length]=n)}),e)},w.reject=function(n,t,r){return w.filter(n,function(n,e,u){return!t.call(r,n,e,u)},r)},w.every=w.all=function(n,t,e){t||(t=w.identity);var u=!0;return null==n?u:g&&n.every===g?n.every(t,e):(A(n,function(n,i,a){return(u=u&&t.call(e,n,i,a))?void 0:r}),!!u)};var E=w.some=w.any=function(n,t,e){t||(t=w.identity);var u=!1;return null==n?u:m&&n.some===m?n.some(t,e):(A(n,function(n,i,a){return u||(u=t.call(e,n,i,a))?r:void 0}),!!u)};w.contains=w.include=function(n,t){return null==n?!1:y&&n.indexOf===y?n.indexOf(t)!=-1:E(n,function(n){return n===t})},w.invoke=function(n,t){var r=o.call(arguments,2),e=w.isFunction(t);return w.map(n,function(n){return(e?t:n[t]).apply(n,r)})},w.pluck=function(n,t){return w.map(n,function(n){return n[t]})},w.where=function(n,t,r){return w.isEmpty(t)?r?null:[]:w[r?"find":"filter"](n,function(n){for(var r in t)if(t[r]!==n[r])return!1;return!0})},w.findWhere=function(n,t){return w.where(n,t,!0)},w.max=function(n,t,r){if(!t&&w.isArray(n)&&n[0]===+n[0]&&65535>n.length)return Math.max.apply(Math,n);if(!t&&w.isEmpty(n))return-1/0;var e={computed:-1/0,value:-1/0};return A(n,function(n,u,i){var a=t?t.call(r,n,u,i):n;a>=e.computed&&(e={value:n,computed:a})}),e.value},w.min=function(n,t,r){if(!t&&w.isArray(n)&&n[0]===+n[0]&&65535>n.length)return Math.min.apply(Math,n);if(!t&&w.isEmpty(n))return 1/0;var e={computed:1/0,value:1/0};return A(n,function(n,u,i){var a=t?t.call(r,n,u,i):n;e.computed>a&&(e={value:n,computed:a})}),e.value},w.shuffle=function(n){var t,r=0,e=[];return A(n,function(n){t=w.random(r++),e[r-1]=e[t],e[t]=n}),e};var k=function(n){return w.isFunction(n)?n:function(t){return t[n]}};w.sortBy=function(n,t,r){var e=k(t);return w.pluck(w.map(n,function(n,t,u){return{value:n,index:t,criteria:e.call(r,n,t,u)}}).sort(function(n,t){var r=n.criteria,e=t.criteria;if(r!==e){if(r>e||r===void 0)return 1;if(e>r||e===void 0)return-1}return n.index<t.index?-1:1}),"value")};var F=function(n,t,r,e){var u={},i=k(t||w.identity);return A(n,function(t,a){var o=i.call(r,t,a,n);e(u,o,t)}),u};w.groupBy=function(n,t,r){return F(n,t,r,function(n,t,r){(w.has(n,t)?n[t]:n[t]=[]).push(r)})},w.countBy=function(n,t,r){return F(n,t,r,function(n,t){w.has(n,t)||(n[t]=0),n[t]++})},w.sortedIndex=function(n,t,r,e){r=null==r?w.identity:k(r);for(var u=r.call(e,t),i=0,a=n.length;a>i;){var o=i+a>>>1;u>r.call(e,n[o])?i=o+1:a=o}return i},w.toArray=function(n){return n?w.isArray(n)?o.call(n):n.length===+n.length?w.map(n,w.identity):w.values(n):[]},w.size=function(n){return null==n?0:n.length===+n.length?n.length:w.keys(n).length},w.first=w.head=w.take=function(n,t,r){return null==n?void 0:null==t||r?n[0]:o.call(n,0,t)},w.initial=function(n,t,r){return o.call(n,0,n.length-(null==t||r?1:t))},w.last=function(n,t,r){return null==n?void 0:null==t||r?n[n.length-1]:o.call(n,Math.max(n.length-t,0))},w.rest=w.tail=w.drop=function(n,t,r){return o.call(n,null==t||r?1:t)},w.compact=function(n){return w.filter(n,w.identity)};var R=function(n,t,r){return A(n,function(n){w.isArray(n)?t?a.apply(r,n):R(n,t,r):r.push(n)}),r};w.flatten=function(n,t){return R(n,t,[])},w.without=function(n){return w.difference(n,o.call(arguments,1))},w.uniq=w.unique=function(n,t,r,e){w.isFunction(t)&&(e=r,r=t,t=!1);var u=r?w.map(n,r,e):n,i=[],a=[];return A(u,function(r,e){(t?e&&a[a.length-1]===r:w.contains(a,r))||(a.push(r),i.push(n[e]))}),i},w.union=function(){return w.uniq(c.apply(e,arguments))},w.intersection=function(n){var t=o.call(arguments,1);return w.filter(w.uniq(n),function(n){return w.every(t,function(t){return w.indexOf(t,n)>=0})})},w.difference=function(n){var t=c.apply(e,o.call(arguments,1));return w.filter(n,function(n){return!w.contains(t,n)})},w.zip=function(){for(var n=o.call(arguments),t=w.max(w.pluck(n,"length")),r=Array(t),e=0;t>e;e++)r[e]=w.pluck(n,""+e);return r},w.object=function(n,t){if(null==n)return{};for(var r={},e=0,u=n.length;u>e;e++)t?r[n[e]]=t[e]:r[n[e][0]]=n[e][1];return r},w.indexOf=function(n,t,r){if(null==n)return-1;var e=0,u=n.length;if(r){if("number"!=typeof r)return e=w.sortedIndex(n,t),n[e]===t?e:-1;e=0>r?Math.max(0,u+r):r}if(y&&n.indexOf===y)return n.indexOf(t,r);for(;u>e;e++)if(n[e]===t)return e;return-1},w.lastIndexOf=function(n,t,r){if(null==n)return-1;var e=null!=r;if(b&&n.lastIndexOf===b)return e?n.lastIndexOf(t,r):n.lastIndexOf(t);for(var u=e?r:n.length;u--;)if(n[u]===t)return u;return-1},w.range=function(n,t,r){1>=arguments.length&&(t=n||0,n=0),r=arguments[2]||1;for(var e=Math.max(Math.ceil((t-n)/r),0),u=0,i=Array(e);e>u;)i[u++]=n,n+=r;return i},w.bind=function(n,t){if(n.bind===j&&j)return j.apply(n,o.call(arguments,1));var r=o.call(arguments,2);return function(){return n.apply(t,r.concat(o.call(arguments)))}},w.partial=function(n){var t=o.call(arguments,1);return function(){return n.apply(this,t.concat(o.call(arguments)))}},w.bindAll=function(n){var t=o.call(arguments,1);return 0===t.length&&(t=w.functions(n)),A(t,function(t){n[t]=w.bind(n[t],n)}),n},w.memoize=function(n,t){var r={};return t||(t=w.identity),function(){var e=t.apply(this,arguments);return w.has(r,e)?r[e]:r[e]=n.apply(this,arguments)}},w.delay=function(n,t){var r=o.call(arguments,2);return setTimeout(function(){return n.apply(null,r)},t)},w.defer=function(n){return w.delay.apply(w,[n,1].concat(o.call(arguments,1)))},w.throttle=function(n,t){var r,e,u,i,a=0,o=function(){a=new Date,u=null,i=n.apply(r,e)};return function(){var c=new Date,l=t-(c-a);return r=this,e=arguments,0>=l?(clearTimeout(u),u=null,a=c,i=n.apply(r,e)):u||(u=setTimeout(o,l)),i}},w.debounce=function(n,t,r){var e,u;return function(){var i=this,a=arguments,o=function(){e=null,r||(u=n.apply(i,a))},c=r&&!e;return clearTimeout(e),e=setTimeout(o,t),c&&(u=n.apply(i,a)),u}},w.once=function(n){var t,r=!1;return function(){return r?t:(r=!0,t=n.apply(this,arguments),n=null,t)}},w.wrap=function(n,t){return function(){var r=[n];return a.apply(r,arguments),t.apply(this,r)}},w.compose=function(){var n=arguments;return function(){for(var t=arguments,r=n.length-1;r>=0;r--)t=[n[r].apply(this,t)];return t[0]}},w.after=function(n,t){return 0>=n?t():function(){return 1>--n?t.apply(this,arguments):void 0}},w.keys=_||function(n){if(n!==Object(n))throw new TypeError("Invalid object");var t=[];for(var r in n)w.has(n,r)&&(t[t.length]=r);return t},w.values=function(n){var t=[];for(var r in n)w.has(n,r)&&t.push(n[r]);return t},w.pairs=function(n){var t=[];for(var r in n)w.has(n,r)&&t.push([r,n[r]]);return t},w.invert=function(n){var t={};for(var r in n)w.has(n,r)&&(t[n[r]]=r);return t},w.functions=w.methods=function(n){var t=[];for(var r in n)w.isFunction(n[r])&&t.push(r);return t.sort()},w.extend=function(n){return A(o.call(arguments,1),function(t){if(t)for(var r in t)n[r]=t[r]}),n},w.pick=function(n){var t={},r=c.apply(e,o.call(arguments,1));return A(r,function(r){r in n&&(t[r]=n[r])}),t},w.omit=function(n){var t={},r=c.apply(e,o.call(arguments,1));for(var u in n)w.contains(r,u)||(t[u]=n[u]);return t},w.defaults=function(n){return A(o.call(arguments,1),function(t){if(t)for(var r in t)null==n[r]&&(n[r]=t[r])}),n},w.clone=function(n){return w.isObject(n)?w.isArray(n)?n.slice():w.extend({},n):n},w.tap=function(n,t){return t(n),n};var I=function(n,t,r,e){if(n===t)return 0!==n||1/n==1/t;if(null==n||null==t)return n===t;n instanceof w&&(n=n._wrapped),t instanceof w&&(t=t._wrapped);var u=l.call(n);if(u!=l.call(t))return!1;switch(u){case"[object String]":return n==t+"";case"[object Number]":return n!=+n?t!=+t:0==n?1/n==1/t:n==+t;case"[object Date]":case"[object Boolean]":return+n==+t;case"[object RegExp]":return n.source==t.source&&n.global==t.global&&n.multiline==t.multiline&&n.ignoreCase==t.ignoreCase}if("object"!=typeof n||"object"!=typeof t)return!1;for(var i=r.length;i--;)if(r[i]==n)return e[i]==t;r.push(n),e.push(t);var a=0,o=!0;if("[object Array]"==u){if(a=n.length,o=a==t.length)for(;a--&&(o=I(n[a],t[a],r,e)););}else{var c=n.constructor,f=t.constructor;if(c!==f&&!(w.isFunction(c)&&c instanceof c&&w.isFunction(f)&&f instanceof f))return!1;for(var s in n)if(w.has(n,s)&&(a++,!(o=w.has(t,s)&&I(n[s],t[s],r,e))))break;if(o){for(s in t)if(w.has(t,s)&&!a--)break;o=!a}}return r.pop(),e.pop(),o};w.isEqual=function(n,t){return I(n,t,[],[])},w.isEmpty=function(n){if(null==n)return!0;if(w.isArray(n)||w.isString(n))return 0===n.length;for(var t in n)if(w.has(n,t))return!1;return!0},w.isElement=function(n){return!(!n||1!==n.nodeType)},w.isArray=x||function(n){return"[object Array]"==l.call(n)},w.isObject=function(n){return n===Object(n)},A(["Arguments","Function","String","Number","Date","RegExp"],function(n){w["is"+n]=function(t){return l.call(t)=="[object "+n+"]"}}),w.isArguments(arguments)||(w.isArguments=function(n){return!(!n||!w.has(n,"callee"))}),"function"!=typeof/./&&(w.isFunction=function(n){return"function"==typeof n}),w.isFinite=function(n){return isFinite(n)&&!isNaN(parseFloat(n))},w.isNaN=function(n){return w.isNumber(n)&&n!=+n},w.isBoolean=function(n){return n===!0||n===!1||"[object Boolean]"==l.call(n)},w.isNull=function(n){return null===n},w.isUndefined=function(n){return n===void 0},w.has=function(n,t){return f.call(n,t)},w.noConflict=function(){return n._=t,this},w.identity=function(n){return n},w.times=function(n,t,r){for(var e=Array(n),u=0;n>u;u++)e[u]=t.call(r,u);return e},w.random=function(n,t){return null==t&&(t=n,n=0),n+Math.floor(Math.random()*(t-n+1))};var M={escape:{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#x27;","/":"&#x2F;"}};M.unescape=w.invert(M.escape);var S={escape:RegExp("["+w.keys(M.escape).join("")+"]","g"),unescape:RegExp("("+w.keys(M.unescape).join("|")+")","g")};w.each(["escape","unescape"],function(n){w[n]=function(t){return null==t?"":(""+t).replace(S[n],function(t){return M[n][t]})}}),w.result=function(n,t){if(null==n)return null;var r=n[t];return w.isFunction(r)?r.call(n):r},w.mixin=function(n){A(w.functions(n),function(t){var r=w[t]=n[t];w.prototype[t]=function(){var n=[this._wrapped];return a.apply(n,arguments),D.call(this,r.apply(w,n))}})};var N=0;w.uniqueId=function(n){var t=++N+"";return n?n+t:t},w.templateSettings={evaluate:/<%([\s\S]+?)%>/g,interpolate:/<%=([\s\S]+?)%>/g,escape:/<%-([\s\S]+?)%>/g};var T=/(.)^/,q={"'":"'","\\":"\\","\r":"r","\n":"n","	":"t","\u2028":"u2028","\u2029":"u2029"},B=/\\|'|\r|\n|\t|\u2028|\u2029/g;w.template=function(n,t,r){var e;r=w.defaults({},r,w.templateSettings);var u=RegExp([(r.escape||T).source,(r.interpolate||T).source,(r.evaluate||T).source].join("|")+"|$","g"),i=0,a="__p+='";n.replace(u,function(t,r,e,u,o){return a+=n.slice(i,o).replace(B,function(n){return"\\"+q[n]}),r&&(a+="'+\n((__t=("+r+"))==null?'':_.escape(__t))+\n'"),e&&(a+="'+\n((__t=("+e+"))==null?'':__t)+\n'"),u&&(a+="';\n"+u+"\n__p+='"),i=o+t.length,t}),a+="';\n",r.variable||(a="with(obj||{}){\n"+a+"}\n"),a="var __t,__p='',__j=Array.prototype.join,"+"print=function(){__p+=__j.call(arguments,'');};\n"+a+"return __p;\n";try{e=Function(r.variable||"obj","_",a)}catch(o){throw o.source=a,o}if(t)return e(t,w);var c=function(n){return e.call(this,n,w)};return c.source="function("+(r.variable||"obj")+"){\n"+a+"}",c},w.chain=function(n){return w(n).chain()};var D=function(n){return this._chain?w(n).chain():n};w.mixin(w),A(["pop","push","reverse","shift","sort","splice","unshift"],function(n){var t=e[n];w.prototype[n]=function(){var r=this._wrapped;return t.apply(r,arguments),"shift"!=n&&"splice"!=n||0!==r.length||delete r[0],D.call(this,r)}}),A(["concat","join","slice"],function(n){var t=e[n];w.prototype[n]=function(){return D.call(this,t.apply(this._wrapped,arguments))}}),w.extend(w.prototype,{chain:function(){return this._chain=!0,this},value:function(){return this._wrapped}})}).call(this);define("Underscore",function(global){return function(){var ret,fn;return ret||global._}}(this));define("ventus/wm/modes/expose",["Underscore","less!../../../css/expose"],function(_){var ExposeMode={register:function(){var self=this;console.log("Expose mode registered.");this.el.on("contextmenu",_.throttle(function(){if(self.mode!=="expose"){self.mode="expose"}else if(self.mode==="expose"){self.mode="default"}return false},1e3))},plug:function(){var floor=Math.floor,ceil=Math.ceil,self=this;var grid=ceil(this.windows.length/2);var maxWidth=floor(this.el.width()/grid);var maxHeight=floor(this.el.height()/2);var scale,left,top,pos;this.el.addClass("expose");for(var z,win,i=0,len=this.windows.length;i<len;i++){win=this.windows[i];win.stamp();if(win.height>win.width){scale=win.height>maxHeight?maxHeight/win.height:1}else{scale=win.width>maxWidth?maxWidth/win.width:1}scale-=.15;pos={x:i%grid*maxWidth,y:(i<grid?0:1)*maxHeight};left=pos.x+floor((maxWidth-scale*win.width)/2);top=pos.y+floor((maxHeight-scale*win.height)/2);win.enabled=false;win.movable=false;win.el.addClass("exposing");win.el.css("transform-origin","0 0");win.el.css("transform","scale("+scale+")");win.el.css("top",top);win.el.css("left",left);win.el.onTransitionEnd(function(){win.el.removeClass("exposing")},this)}this.overlay=true;this.el.one("click",function(){self.mode="default"})},unplug:function(){var space=this.el;for(var z,win,i=this.windows.length;i--;){win=this.windows[i];win.restore();win.el.css("transform","scale(1)");win.el.css("transform-origin","50% 50%");var removeTransform=function(win){return function(){this.el.removeClass("expose");win.el.css("transform","")}}(win);this.el.onTransitionEnd(removeTransform,this);win.movable=true;win.enabled=true}this.overlay=false},actions:{focus:function(win){},close:function(){this.mode="expose"},select:function(win,e){this.mode="default";win.focus()}}};return ExposeMode});define("ventus/wm/modes/fullscreen",["less!../../../css/fullscreen"],function(){var FullscreenMode={register:function(){console.log("Fullscreen mode registered.")},plug:function(){this.el.addClass("fullscreen");for(var win,i=0,len=this.windows.length;i<len;i++){win=this.windows[i];win.move(0,0);win.el.css("-webkit-transform","translate3d(0, 0, 0);");win.resize(this.el.width(),this.el.height())}},unplug:function(){for(var win,i=this.windows.length;i--;){win=this.windows[i];win.restore();win.el.css("transform","scale(1)");win.el.css("transform-origin","50% 50%");var removeTransform=function(win){return function(){this.el.removeClass("fullscreen");win.el.css("transform","")}}(win);this.el.onTransitionEnd(removeTransform,this);win.movable=true;win.resizable=true;win.enabled=true}this.overlay=false},actions:{focus:function(){},close:function(){this.mode="expose"},select:function(win){this.mode="default";win.focus()}}};return FullscreenMode});define("ventus/wm/windowmanager",["require","$","ventus/wm/window","ventus/core/view","ventus/wm/modes/default","ventus/wm/modes/expose","ventus/wm/modes/fullscreen"],function(require){var $=require("$");var Window=require("ventus/wm/window");var view=require("ventus/core/view");var DefaultMode=require("ventus/wm/modes/default");var ExposeMode=require("ventus/wm/modes/expose");var FullscreenMode=require("ventus/wm/modes/fullscreen");var WindowManager=function(){this.el=view('<div class="wm-space"><div class="wm-overlay" /></div>');$("#desktopZone").append(this.el);this.$overlay=this.el.find(".wm-overlay");this.$overlay.css("z-index","1000");this.actions.forEach(function(value){this[value]=function(action){return function(){if(this.currentMode.actions[action])this.currentMode.actions[action].apply(this,arguments)}}.call(this,value)},this);for(var mode in this.modes){if(this.modes.hasOwnProperty(mode)&&this.modes[mode].register){this.modes[mode].register.apply(this)}}this.windows=[];this.active=null;this.mode="default";this.createWindow.fromQuery=this.createWindow.fromQuery.bind(this);this.createWindow.fromElement=this.createWindow.fromElement.bind(this)};WindowManager.prototype={actions:["focus","blur","close","maximize","minimize","restore","select"],modes:{"default":DefaultMode,expose:ExposeMode,fullscreen:FullscreenMode},set mode(value){var mode=this.modes[value];if(!mode||this._mode===value)return;if(this._mode&&this.currentMode.unplug)this.currentMode.unplug.apply(this);if(mode.plug)mode.plug.apply(this);this._mode=value},get mode(){return this._mode},get currentMode(){return this.modes[this._mode]},set overlay(value){this.$overlay.css("opacity",value?.8:0);this._overlay=value},get overlay(){return this._overlay},createWindow:function(options){var win=new Window(options);this.mode="default";win.signals.on("focus",this._focus,this);win.signals.on("blur",this._blur,this);win.signals.on("close",this._close,this);this.actions.forEach(function(action){win.signals.on(action,this[action],this)},this);this.windows.push(win);$(".wm-space").show();win.space=this.el;win.focus();return win},_focus:function(win){var currentZ,baseZ=1e4,maxZ=baseZ+1e4,index;if(this.active&&this.active===win)return;if(this.active){currentZ=this.active.z;this.active.blur()}else{currentZ=baseZ}index=this.windows.indexOf(win);this.windows.splice(index,1);this.windows.push(win);win.z=currentZ+1;if(currentZ>maxZ+this.windows.length){for(var z,i=this.windows.length;i--;){z=this.windows[i].z;this.windows[i].z=baseZ+(z-maxZ)}}this.active=win},_blur:function(win){if(this.active===win)this.active=null},_close:function(win){var id=this.windows.indexOf(win),len;if(id===-1){console.log("Trying to close a window that doesn't exist in this window manager");return}this.windows.splice(id,1);len=this.windows.length;if(this.active&&this.active===win){this.active=len!==0?this.windows[len-1]:null;if(this.active)this.active.focus()}}};WindowManager.prototype.createWindow.fromQuery=function(selector,options){options.content=view(selector);return this.createWindow(options)};WindowManager.prototype.createWindow.fromElement=function(element,options){options.content=view(element);return this.createWindow(options)};return WindowManager});define("ventus",["require","ventus/wm/windowmanager","ventus/wm/window"],function(require){return{version:"0.1",browser:{animationEventName:function(){var style=document.body.style;var event=null;if(style.animation==="")event="animationend";else if(style.MozAnimation==="")event="mozAnimationEnd";else if(style.webkitAnimation==="")event="webkitAnimationEnd";return event}},WindowManager:require("ventus/wm/windowmanager"),Window:require("ventus/wm/window")}});define("$",function(){return $});define("handlebars",function(){return Handlebars});return require("ventus")});var PeerConnection=window.PeerConnection||window.webkitPeerConnection00||window.webkitRTCPeerConnection||window.mozRTCPeerConnection;var URL=window.URL||window.webkitURL||window.msURL||window.oURL;var getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;var nativeRTCIceCandidate=window.mozRTCIceCandidate||window.RTCIceCandidate;var nativeRTCSessionDescription=window.mozRTCSessionDescription||window.RTCSessionDescription;if(navigator.webkitGetUserMedia){if(!webkitMediaStream.prototype.getVideoTracks){webkitMediaStream.prototype.getVideoTracks=function(){return this.videoTracks};webkitMediaStream.prototype.getAudioTracks=function(){return this.audioTracks}}if(!webkitRTCPeerConnection.prototype.getLocalStreams){webkitRTCPeerConnection.prototype.getLocalStreams=function(){return this.localStreams};webkitRTCPeerConnection.prototype.getRemoteStreams=function(){return this.remoteStreams}}}else if(navigator.mozGetUserMedia){if(!MediaStream.prototype.getVideoTracks){MediaStream.prototype.getVideoTracks=function(){return[]}}if(!MediaStream.prototype.getAudioTracks){MediaStream.prototype.getAudioTracks=function(){return[]}}}(function(){var rtc;if("undefined"===typeof module){rtc=this.rtc={}}else{rtc=module.exports={}}rtc.debug=false;rtc._socket=null;rtc._me=null;rtc.room=null;rtc._events={};rtc.iceServers=[];rtc.pingInterval=null;rtc.connected=false;rtc.reset=function(){if(rtc._socket)rtc._socket.close();if(rtc.pingInterval)window.clearInterval(rtc.pingInterval);rtc._me=null;rtc._events={};rtc.pingInterval=null;rtc.connected=false};rtc.uniqueon=function(eventName,callback){if(rtc._events[eventName]){delete rtc._events[eventName]}rtc.on(eventName,callback)};rtc.deleteEvent=function(eventName){if(rtc._events[eventName]){delete rtc._events[eventName]}};rtc.on=function(eventName,callback){rtc._events[eventName]=rtc._events[eventName]||[];rtc._events[eventName].push(callback)};rtc.fire=function(eventName,_){var events=rtc._events[eventName];var args=Array.prototype.slice.call(arguments,1);if(!events){return}for(var i=0,len=events.length;i<len;i++){events[i].apply(null,args)}};rtc.producedPeerConnections={};rtc.receivedPeerConnections={};rtc.connections=[];rtc.streams=[];rtc.numStreams=0;rtc.initializedStreams=0;rtc.dataChannels={};rtc.sources=["audio","video","screen","filedata"];rtc.sourceSdpConstraints={audio:{mandatory:{OfferToReceiveAudio:true,OfferToReceiveVideo:false}},video:{mandatory:{OfferToReceiveAudio:false,OfferToReceiveVideo:true}},screen:{mandatory:{OfferToReceiveAudio:false,OfferToReceiveVideo:true}},filedata:{mandatory:{OfferToReceiveAudio:false,OfferToReceiveVideo:false}}};rtc.pc_constraints={optional:[{DtlsSrtpKeyAgreement:true}]};rtc.chromeDesktopExtensionId="lalali";rtc.ping=function(){rtc._socket.send(JSON.stringify({eventName:"ping",data:{room:rtc.room}}))};rtc.peerListUpdated=function(room){rtc._socket.send(JSON.stringify({eventName:"peer_list_updated",data:{room:room}}))};rtc.sendChatMessage=function(room,message){rtc._socket.send(JSON.stringify({eventName:"chat_message",data:{room:room,text:message}}))};rtc.sendChatTyping=function(room){rtc._socket.send(JSON.stringify({eventName:"chat_typing",data:{room:room}}))};rtc.connect=function(server,room,password,reload){room=room||"";rtc._socket=new WebSocket(server);rtc.room=room;rtc._socket.onopen=function(){rtc.askForUpdateConfig=function(room){rtc._socket.send(JSON.stringify({eventName:"update_server_config",data:{room:room}}))};rtc.on("get_updated_config",function(data){rtc.iceServers=data.iceServers;rtc.chromeDesktopExtensionId=data.chromeDesktopExtensionId;if(rtc.debug)console.log("serverList updated"+JSON.stringify(rtc.iceServers));if(rtc.debug)console.log("extension id "+rtc.chromeDesktopExtensionId)});rtc.askForUpdateConfig();rtc.tryToConnect=function(troom,tpassword,treload){if(rtc.iceServers!=undefined&&rtc.iceServers.length>0){rtc._socket.send(JSON.stringify({eventName:"join_room",data:{room:troom,pwd:tpassword,reload:treload}}));rtc.pingInterval=setInterval("rtc.ping();",5e3)}else{setTimeout(function(){rtc.tryToConnect(troom,tpassword,treload)},1e3)}};rtc.tryToConnect(room,password,reload);rtc._socket.onmessage=function(msg){var json=JSON.parse(msg.data);rtc.fire(json.eventName,json.data)};rtc._socket.onerror=function(err){console.error("onerror");console.error(err);location.reload()};rtc._socket.onclose=function(data){var id=rtc._socket.id;rtc.fire("disconnect stream",id);window.clearInterval(rtc.pingInterval);rtc.connections={};rtc.receivedPeerConnections={};rtc.producedPeerConnections={};rtc._socket=null};rtc.on("ready",function(mediatype,maxBitrate){rtc.createPeerConnections(mediatype);rtc.addStreams(mediatype);rtc.sendOffers(mediatype,maxBitrate)});rtc.on("get_peers",function(data){rtc.connections=data.connections;rtc._me=data.you;rtc.fire("connections",rtc.connections)});rtc.on("receive_ice_candidate",function(data){var candidate=new nativeRTCIceCandidate({candidate:data.candidate,sdpMLineIndex:data.label});var peerConnections=data.produced?rtc.receivedPeerConnections:rtc.producedPeerConnections;if(!peerConnections[data.socketId]){peerConnections[data.socketId]={}}if(!peerConnections[data.socketId][data.mediatype]){if(rtc.debug)console.log("Peer not ready, so storing ICE candidate for a while");if(!peerConnections[data.socketId]["temp_"+data.mediatype])peerConnections[data.socketId]["temp_"+data.mediatype]=[];peerConnections[data.socketId]["temp_"+data.mediatype].push(candidate);return}else{peerConnections[data.socketId][data.mediatype].addIceCandidate(candidate,function(){if(rtc.debug)console.log("ICE candidate added")},function(error){if(rtc.debug)console.log("Error adding an ICE candidate "+JSON.stringify(error))})}rtc.fire("receive ice candidate",candidate)});rtc.on("new_peer_connected",function(data){var id=data.socketId;rtc.connections.push(id);var sendDelayedOffer=function(id,mediatype){setTimeout(function(){rtc.sendOffer(id,mediatype);if(rtc.debug)console.log("Sending to "+id+" a "+mediatype+"offer")},2e3)};for(var i=0;i<rtc.streams.length;i++){var stream=rtc.streams[i].mediastream;var mediatype=rtc.streams[i].mediatype;var pc=rtc.createPeerConnection(id,mediatype,true);pc.addStream(stream);sendDelayedOffer(id,mediatype)}});rtc.on("remove_peer_connected",function(data){var id=data.socketId;rtc.fire("disconnect stream",id);delete rtc.dataChannels[id];for(var i=0;i<rtc.sources.length&&rtc.receivedPeerConnections[id];i++){if(rtc.receivedPeerConnections[id][rtc.sources[i]])rtc.dropPeerConnection(id,rtc.sources[i],false)}for(var i=0;i<rtc.sources.length&&rtc.producedPeerConnections[id];i++){if(rtc.producedPeerConnections[id][rtc.sources[i]])rtc.dropPeerConnection(id,rtc.sources[i],true)}for(var j=0;j<rtc.connections.length;j++){if(rtc.connections[j]===id){rtc.connections.splice(j,1);return}}});rtc.on("receive_offer",function(data){if(data.mediatype.indexOf("filedata")!=-1){rtc.fire("receive file offer",data)}else{rtc.receiveOffer(data.socketId,data.sdp,data.mediatype);
rtc.fire("receive offer",data)}});rtc.on("receive_answer",function(data){rtc.receiveAnswer(data.socketId,data.sdp,data.mediatype);rtc.fire("receive answer",data)});rtc.fire("connect")}};rtc.sendOffers=function(mediatype,maxBitrate){for(var i=0,len=rtc.connections.length;i<len;i++){var socketId=rtc.connections[i];rtc.sendOffer(socketId,mediatype,maxBitrate)}};rtc.onClose=function(data){rtc.on("close_stream",function(){rtc.fire("close_stream",data)})};rtc.createPeerConnections=function(mediatype){for(var i=0;i<rtc.connections.length;i++){rtc.createPeerConnection(rtc.connections[i],mediatype,true)}};rtc.createPeerConnection=function(id,mediatype,produced,requestId){var peerConnections=produced?rtc.producedPeerConnections:rtc.receivedPeerConnections;var config=rtc.pc_constraints;if(!peerConnections[id]){peerConnections[id]={}}peerConnections[id][mediatype]=new PeerConnection({iceServers:rtc.iceServers},config);var pc=peerConnections[id][mediatype];pc.onicecandidate=function(event){if(event.candidate&&event.candidate.candidate){if(rtc.debug)console.log("Sending an ICE candidate");rtc._socket.send(JSON.stringify({eventName:"send_ice_candidate",data:{label:event.candidate.sdpMLineIndex,candidate:event.candidate.candidate,socketId:id,mediatype:mediatype,produced:produced}}));rtc.fire("ice candidate",event.candidate)}};pc.onopen=function(){rtc.fire("peer connection opened")};pc.onaddstream=function(event){var tries=0;var tryStream=function(){if(pc!=null){if(pc.iceConnectionState==="completed"||pc.iceConnectionState=="connected"){if(rtc.debug)console.log("the connection was completed send the data");rtc.fire("add remote stream",event.stream,id,mediatype)}else{setTimeout(function(){if(rtc.debug)console.log("Remote stream not added yet. Try : "+tries);tries++;if(tries<10)tryStream()},1500)}}};tryStream()};pc.oniceconnectionstatechange=function(event){if(rtc.debug)console.log("User id: "+id+" changed : "+pc.iceConnectionState+" mediatype"+mediatype)};pc.ondatachannel=function(evt){if(rtc.debug)console.log("data channel connecting "+id);rtc.addDataChannel(id,evt.channel,requestId,mediatype)};return pc};rtc.sendOffer=function(socketId,mediatype,maxBitrate,requestId,token){var pc=rtc.producedPeerConnections[socketId][mediatype];var constraints={optional:[],mandatory:{MozDontOfferDataChannel:true}};if(navigator.webkitGetUserMedia){for(var prop in constraints.mandatory){if(prop.indexOf("Moz")!=-1){delete constraints.mandatory[prop]}}}var sourceType="";var sourceType=mediatype.indexOf("filedata")!=-1?"filedata":mediatype;var sdpConstraints=rtc.sourceSdpConstraints[sourceType];constraints=mergeConstraints(constraints,sdpConstraints);pc.createOffer(function(session_description){if(maxBitrate){session_description.sdp=changeBitrate(session_description.sdp,maxBitrate);pc.tempSession_description=session_description}pc.setLocalDescription(session_description,function(){rtc._socket.send(JSON.stringify({eventName:"send_offer",data:{socketId:socketId,sdp:session_description,mediatype:mediatype,room:rtc.room,requestId:requestId,token:token}}))},function(){if(rtc.debug)console.log("Error setting the local description")})},function(){if(rtc.debug)console.log("Error creating the offer")},sdpConstraints)};rtc.receiveOffer=function(socketId,sdp,mediatype,requestId){var pcs=rtc.receivedPeerConnections[socketId];if(!rtc.receivedPeerConnections[socketId]){rtc.receivedPeerConnections[socketId]={}}var pc=rtc.receivedPeerConnections[socketId][mediatype];if(pc!=undefined){if(rtc.debug)console.log("Destroyed previous connection");rtc.dropPeerConnection(socketId,mediatype,false)}if(rtc.debug)console.log("Create a new one connection");pc=rtc.createPeerConnection(socketId,mediatype,false,requestId);if(rtc.debug)console.log("Send a new annser");rtc.sendAnswer(socketId,sdp,mediatype)};rtc.sendAnswer=function(socketId,sdp,mediatype){var pc=rtc.receivedPeerConnections[socketId][mediatype];var sdpConstraints=rtc.sourceSdpConstraints[mediatype];pc.setRemoteDescription(new nativeRTCSessionDescription(sdp),function(){if(rtc.receivedPeerConnections[socketId]["temp_"+mediatype]){for(var i=0;i<rtc.receivedPeerConnections[socketId]["temp_"+mediatype].length;i++){var storedIceCandidate=rtc.receivedPeerConnections[socketId]["temp_"+mediatype][i];pc.addIceCandidate(storedIceCandidate,function(){if(rtc.debug)console.log("ICE candidate added")},function(error){if(rtc.debug)console.log("Error adding ICE candiate"+JSON.stringify(error))})}}pc.createAnswer(function(session_description){pc.setLocalDescription(session_description,function(){rtc._socket.send(JSON.stringify({eventName:"send_answer",data:{socketId:socketId,sdp:session_description,mediatype:mediatype}}))},function(){if(rtc.debug)console.log("Error setting local description ")});var offer=pc.remoteDescription},function(){if(rtc.debug)console.log("Error creating the anwer ")},sdpConstraints)},function(){if(rtc.debug)console.log("Error setting retmote description ")})};rtc.receiveAnswer=function(socketId,sdp,mediatype){var pc=rtc.producedPeerConnections[socketId][mediatype];pc.setRemoteDescription(new nativeRTCSessionDescription(sdp),function(){setTimeout(function(){if(rtc.debug)console.log("PCICE::::"+pc.iceConnectionState);if(pc.iceConnectionState=="checking"){if(rtc.debug)console.log("Seems that the state is stalled !!");rtc.dropPeerConnection(socketId,mediatype,true);rtc.createPeerConnection(socketId,mediatype,true);rtc.addStream(socketId,mediatype);rtc.sendOffer(socketId,mediatype)}},1e4)},function(){if(rtc.debug)console.log("Error setting remote description")})};rtc.createStream=function(mediatype,options,onSuccess,onFail,maxBitrate){onSuccess=onSuccess||function(){};onFail=onFail||function(){};if(getUserMedia){var startStream=function(){rtc.numStreams++;getUserMedia.call(navigator,options,function(stream){var streamObj={};streamObj.mediastream=stream;streamObj.mediatype=mediatype;rtc.streams.push(streamObj);rtc.initializedStreams++;onSuccess(stream);if(rtc.initializedStreams===rtc.numStreams){rtc.fire("ready",mediatype,maxBitrate)}},function(error){onFail(error)})};if(mediatype==="screen"){getScreenId(rtc.chromeDesktopExtensionId,function(error,sourceId,screen_constraints){if(error)onFail(error);if(!error&&sourceId){options.video.mandatory.chromeMediaSource="desktop";options.video.mandatory.chromeMediaSourceId=sourceId}startStream()})}else{startStream()}}else{alert("webRTC is not yet supported in this browser.")}};rtc.addStreams=function(mediatype){for(var i=0;i<rtc.streams.length;i++){var streamObj=rtc.streams[i];if(streamObj.mediatype===mediatype){for(var connection in rtc.producedPeerConnections){rtc.producedPeerConnections[connection][mediatype].addStream(streamObj.mediastream)}}}};rtc.addStream=function(connectionId,mediatype){for(var i=0;i<rtc.streams.length;i++){var streamObj=rtc.streams[i];if(streamObj.mediatype===mediatype){rtc.producedPeerConnections[connectionId][mediatype].addStream(streamObj.mediastream)}}};rtc.sendMessage=function(id,mediatype,message,requestId,token){var mediatypefile=mediatype+"_"+requestId;if(!rtc.dataChannels[id]||!rtc.dataChannels[id][mediatypefile]){rtc.createPeerConnection(id,mediatypefile,true);rtc.createDataChannel(id,requestId,mediatypefile);rtc.sendOffer(id,mediatypefile,undefined,requestId,token)}var channel=rtc.dataChannels[id][mediatypefile].channel;var queue=rtc.dataChannels[id][mediatypefile].queue;var state=rtc.dataChannels[id][mediatypefile].state;if(message)queue.push(message);if(state==="ready"){var eventName="data stream open "+id+" "+mediatypefile;if(channel.readyState==="open"){while(queue.length>0){try{var object=queue[0];channel.send(JSON.stringify(object));queue.shift()}catch(event){rtc.dataChannels[id][mediatypefile].state="paused";setTimeout(function(){rtc.dataChannels[id][mediatypefile].state="ready";rtc.sendMessage(id,mediatype,undefined,requestId,token)},50)}}}else if(!rtc._events[eventName]){rtc.on(eventName,function(openedchannel){rtc.deleteEvent(eventName);rtc.dataChannels[id][mediatypefile].state="ready";rtc.sendMessage(id,mediatype,undefined,requestId,token)})}}};rtc.createDataChannel=function(id,requestId,mediatypefile){if(!id)throw new Error("attempt to createDataChannel with unknown id");var userPeerList=rtc.producedPeerConnections[id];var pc=undefined;if(userPeerList&&userPeerList[mediatypefile]){pc=userPeerList[mediatypefile]}else{throw new Error("attempt to createDataChannel without peerConnection")}label=mediatypefile||String(id);var options={reliable:false};var channel;try{if(rtc.debug)console.log("createDataChannel "+id+" "+mediatypefile);channel=pc.createDataChannel(label,options)}catch(error){if(rtc.debug)console.log("seems that DataChannel is NOT actually supported!");throw error}return rtc.addDataChannel(id,channel,requestId,mediatypefile)};rtc.addDataChannel=function(id,channel,requestId,mediatypefile){channel.onopen=function(){if(rtc.debug)console.log("data stream open "+mediatypefile);rtc.fire("data stream open "+id+" "+mediatypefile,channel)};channel.onclose=function(event){delete rtc.dataChannels[id][mediatypefile];if(rtc.debug)console.log("data stream close ");rtc.fire("data stream close",channel)};channel.onmessage=function(event){if(rtc.debug)console.log("data stream message "+id+" "+event.data);rtc.fire("data stream data",channel,id,requestId,mediatypefile,event.data)};channel.onerror=function(err){if(rtc.debug)console.log("data stream error "+id+": "+err);rtc.fire("data stream error",channel,err)};if(!rtc.dataChannels[id]){rtc.dataChannels[id]={}}rtc.dataChannels[id][mediatypefile]={channel:channel,queue:[],state:"ready"};return channel};rtc.dropPeerConnection=function(connectionId,mediatype,produced){var peerConnections=produced?rtc.producedPeerConnections:rtc.receivedPeerConnections;if(peerConnections[connectionId]){if(peerConnections[connectionId][mediatype]){peerConnections[connectionId][mediatype].close();delete peerConnections[connectionId][mediatype]}if(!(peerConnections[connectionId].audio||peerConnections[connectionId].video||peerConnections[connectionId].screen)){delete peerConnections[connectionId]}}};rtc.removeStream=function(room,mediatype){for(var i=0;i<rtc.streams.length;i++){var stream=rtc.streams[i];if(mediatype===stream.mediatype){stream.mediastream.stop();for(var j=0;j<rtc.connections.length;j++){var connectionId=rtc.connections[j];rtc.dropPeerConnection(connectionId,mediatype,true)}rtc.streamClosed(room,mediatype);rtc.streams.splice(i,1);rtc.numStreams--;rtc.initializedStreams--;return}}};rtc.attachStream=function(stream,element){if(typeof element==="string")element=document.getElementById(element);if(navigator.mozGetUserMedia){if(rtc.debug)console.log("Attaching media stream");element.mozSrcObject=stream;element.play()}else{element.src=webkitURL.createObjectURL(stream)}element.play()};rtc.updateOwnerData=function(roomId,ownerName,ownerAvatar,status,access){rtc._socket.send(JSON.stringify({eventName:"update_owner_data",data:{room:roomId,owner_name:ownerName,owner_avatar:ownerAvatar,status:status,access:access}}))};rtc.streamClosed=function(roomId,mediatype){rtc._socket.send(JSON.stringify({eventName:"stream_closed",data:{room:roomId,mediatype:mediatype}}))};rtc.askForScreen=function(room,connectionId,source){rtc._socket.send(JSON.stringify({eventName:"ask_for_sharing",data:{room:room,source:source,connectionId:connectionId}}))};rtc.askForStopScreen=function(room,connectionId,source){rtc._socket.send(JSON.stringify({eventName:"ask_for_stop_sharing",data:{connectionId:connectionId,room:room,source:source}}))};rtc.askForAcceptFiles=function(room,fileOfferId,fileOffer){var filesInfo={};for(fileind in fileOffer.files){var procFile=fileOffer.files[fileind];filesInfo[fileind]={id:fileind,name:procFile.file.name,size:procFile.file.size}}rtc._socket.send(JSON.stringify({eventName:"ask_for_accept_files",data:{connectionId:fileOffer.destinationId,requestId:fileOfferId,room:room,filesinfo:filesInfo}}))};rtc.acceptFilesRequest=function(room,connectionId,requestId,token){rtc._socket.send(JSON.stringify({eventName:"accept_files_request",data:{connectionId:connectionId,room:room,requestId:requestId,token:token}}))};rtc.fileDownloaded=function(room,connectionId,requestId,fileid){rtc._socket.send(JSON.stringify({eventName:"file_download_completed",data:{connectionId:connectionId,room:room,requestId:requestId,fileid:fileid}}))};rtc.cancelFile=function(room,connectionId,requestId,fileid,token,direction){rtc._socket.send(JSON.stringify({eventName:"file_canceled",data:{connectionId:connectionId,room:room,requestId:requestId,fileid:fileid,token:token,direction:direction}}))};rtc.allRequestCompleted=function(room,connectionId,requestId){rtc._socket.send(JSON.stringify({eventName:"files_request_completed",data:{connectionId:connectionId,room:room,requestId:requestId}}))};rtc.fileRequestFailed=function(room,connectionId,requestId,error){rtc._socket.send(JSON.stringify({eventName:"files_request_error",data:{connectionId:connectionId,room:room,requestId:requestId}}))};rtc.moveRoom=function(toRoom,fromRoom,users){rtc.room=toRoom;rtc._socket.send(JSON.stringify({eventName:"move_room",data:{toRoom:toRoom,fromRoom:fromRoom,list:users}}))};rtc.reportErrorToOwner=function(room,origin,type){rtc._socket.send(JSON.stringify({eventName:"error_to_owner",data:{room:room,origin:origin,type:type}}))};rtc.reportErrorToUser=function(room,origin,type){rtc._socket.send(JSON.stringify({eventName:"error_to_user",data:{room:room,origin:origin,type:type}}))}}).call(this);function preferOpus(sdp){var sdpLines=sdp.split("\r\n");var mLineIndex=null;for(var i=0;i<sdpLines.length;i++){if(sdpLines[i].search("m=audio")!==-1){mLineIndex=i;break}}if(mLineIndex===null)return sdp;for(var j=0;j<sdpLines.length;j++){if(sdpLines[j].search("opus/48000")!==-1){var opusPayload=extractSdp(sdpLines[j],/:(\d+) opus\/48000/i);if(opusPayload)sdpLines[mLineIndex]=setDefaultCodec(sdpLines[mLineIndex],opusPayload);break}}sdpLines=removeCN(sdpLines,mLineIndex);sdp=sdpLines.join("\r\n");return sdp}function extractSdp(sdpLine,pattern){var result=sdpLine.match(pattern);return result&&result.length==2?result[1]:null}function setDefaultCodec(mLine,payload){var elements=mLine.split(" ");var newLine=[];var index=0;for(var i=0;i<elements.length;i++){if(index===3)newLine[index++]=payload;if(elements[i]!==payload)newLine[index++]=elements[i]}return newLine.join(" ")}function removeCN(sdpLines,mLineIndex){var mLineElements=sdpLines[mLineIndex].split(" ");for(var i=sdpLines.length-1;i>=0;i--){var payload=extractSdp(sdpLines[i],/a=rtpmap:(\d+) CN\/\d+/i);if(payload){var cnPos=mLineElements.indexOf(payload);if(cnPos!==-1){mLineElements.splice(cnPos,1)}sdpLines.splice(i,1)}}sdpLines[mLineIndex]=mLineElements.join(" ");return sdpLines}function changeBitrate(sdp,bitrate){sdp=sdp.replace(/a=mid:video\r\n/g,"a=mid:video\r\nb=AS:"+bitrate+"\r\n");return sdp}function mergeConstraints(cons1,cons2){var merged=cons1;for(var name in cons2.mandatory){merged.mandatory[name]=cons2.mandatory[name]}merged.optional.concat(cons2.optional);return merged}angular.module("mean.rooms").factory("ChatService",["$timeout","UIHandler","Rooms",function($timeout,UIHandler,Rooms){return function(){var uiHandler=UIHandler;this.welcomePublished=false;var self=this;var room=new Rooms({});this.timeAgo=function($scope,t){var t2=(new Date).getTime();var t1=t.getTime();var secs=parseInt((t2-t1)/1e3);var mins=parseInt((t2-t1)/(1e3*60));return $scope.resourceBundle.timeprep+(secs>59?mins+$scope.resourceBundle.minago:secs+$scope.resourceBundle.secago)};this.longTimeAgo=function(newTime,lastTime){var t2=newTime.getTime();var t1=lastTime.getTime();var secs=parseInt((t2-t1)/1e3);return secs>15};this.youtube=function(url,vid){return'<iframe src="//www.youtube.com/embed/'+vid+'" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>'};this.link=function(url){return'<a href="'+url+'" target="_blank">'+url+"</a>"};this.parseUrls=function(txt){return txt.replace(/https?:\/\/www\.youtube\.com\/watch\?v=([^\s]+)/g,this.youtube).replace(/https?:\/\/youtu\.be\/([^\s]+)/g,this.youtube).replace(/(https?:\/\/[^\s]+)/g,this.link)};this.getHtml=function($scope,data){if(data.id==$scope.global.bot){return this.parseUrls(data.text)}return this.parseUrls(data.text.replace(/&/g,"&amp;").replace(/</g,"&lt;"))};this.addToQueue=function(data){if(!uiHandler.messageQueue){uiHandler.messageQueue=[]}uiHandler.messageQueue.push(data)};this.processQueue=function(){if(uiHandler.messageQueue){for(var j=0;j<uiHandler.messageQueue.length;j+=1){rtc.fire("chat_message",uiHandler.messageQueue[j])}uiHandler.messageQueue=[]}};this.addNewMessage=function($scope,data){var msgTime=new Date(data.time);var messageIndex=uiHandler.messages.length-1;if(uiHandler.messages.length==0||uiHandler.messages[messageIndex].id!=data.id||this.longTimeAgo(msgTime,uiHandler.messages[messageIndex].time)){var m={"class":data.id==$scope.global.own?"self":"other",id:data.id,time:msgTime,istyping:!data.text,list:[]};if(data.text){this.checkTypingIcons(data.id);m.list.push(this.getHtml($scope,data))}uiHandler.messages.push(m)}else{if(!data.text){uiHandler.messages[messageIndex].istyping=true}else{uiHandler.messages[messageIndex].istyping=false;uiHandler.messages[messageIndex].list.push(this.getHtml($scope,data))}}};this.checkTypingIcons=function(id){var nowTime=new Date;for(var i=0;i<uiHandler.messages.length;i+=1){if(uiHandler.messages[i]===id||this.longTimeAgo(nowTime,uiHandler.messages[i].time)){uiHandler.messages[i].istyping=false;if(uiHandler.messages[i].list.length===0){uiHandler.messages.splice(i,1)}}}if(!id){$timeout(function(){self.checkTypingIcons()},2e3)}};this.alertChatStatus=function($scope,accessChat){this.addNewMessage($scope,{id:$scope.global.bot,text:$scope.resourceBundle["chat"+accessChat],time:new Date})};this.alertNotConnected=function($scope){this.addNewMessage($scope,{id:$scope.global.bot,text:$scope.resourceBundle.notconnected,time:new Date})};this.init=function($scope,chatmessages){uiHandler.messages=[];uiHandler.audible=true;$scope.enableAudio=function(){uiHandler.audible=!uiHandler.audible};$scope.toggleChat=function(){uiHandler.chat_class=uiHandler.chat_class=="collapsed"?"":"collapsed";uiHandler.helpchat_class=uiHandler.helpchat_class=="showed"?"":"showed";uiHandler.dash_chat=uiHandler.chat_class=="collapsed"?"chat_collapsed":""};$scope.sendTyping=function(){if(!uiHandler.isowner&&uiHandler.passNeeded){chatService.alertNotConected($scope);return}if(uiHandler.messageText){var now=new Date;if(!self.lastTyping||self.longTimeAgo(now,self.lastTyping)){self.lastTyping=now;rtc.sendChatTyping($scope.global.roomId)}}};$scope.sendMessage=function(){if(!uiHandler.isowner&&uiHandler.passNeeded){chatService.alertNotConected($scope);return}if(uiHandler.messageText){rtc.sendChatMessage($scope.global.roomId,uiHandler.messageText);uiHandler.messageText=""}};angular.element(document.querySelector("#chat_discussion")).on("scroll",function(evt){if(!evt.target.scrollTop&&uiHandler.chatPage>0&&!uiHandler.chatLoading){uiHandler.chatLoading=true;setTimeout(function(){room.chatPage($scope.global.roomId,uiHandler.chatPage,function(rdo){var previousMessages=uiHandler.messages;uiHandler.chatPage=rdo.page;uiHandler.messages=[];for(var i=0;i<rdo.chat.length;i+=1){self.addNewMessage($scope,rdo.chat[i])}for(var j=0;j<previousMessages.length;j+=1){uiHandler.messages.push(previousMessages[j])}uiHandler.chatLoading=false;self.processQueue()})},1500)}});rtc.uniqueon("chat_message",function(data){if(uiHandler.chatLoading){self.addToQueue(data);return}if((uiHandler.helpchat_class=="showed"||!uiHandler.focused)&&uiHandler.audible&&data.id!=rtc._me){var readText=$scope.connectedUsers()>1?$scope.getUser(data.id).name+", "+data.text:data.text;var audio=document.getElementById("audiotts")?document.getElementById("audiotts"):document.createElement("audio");audio.setAttribute("id","audiotts");audio.setAttribute("src","/chat/talk?text="+encodeURIComponent(readText));audio.load();audio.play()}self.addNewMessage($scope,data);uiHandler.safeApply($scope,function(){})});rtc.uniqueon("chat_typing",function(data){self.addNewMessage($scope,data);uiHandler.safeApply($scope,function(){})});if(chatmessages&&uiHandler.messages.length==0){for(var j=0;j<chatmessages.length;j++){this.addNewMessage($scope,chatmessages[j])}}$scope.timeAgo=function(t){return self.timeAgo($scope,t)};if(!self.welcomePublished){$timeout(function(){self.addNewMessage($scope,{id:$scope.global.bot,text:$scope.resourceBundle.welcomechatmess,time:new Date});self.addNewMessage($scope,{id:$scope.global.bot,text:$scope.resourceBundle.helpuschatmess,time:new Date});self.welcomePublished=true},5e3);$timeout(function(){self.checkTypingIcons()},2e3)}}}}]).filter("to_trusted",["$sce",function($sce){return function(text){return $sce.trustAsHtml(text)}}]);angular.module("mean.rooms").factory("FileService",["$sce","UIHandler",function($sce,UIHandler){return function(){var uiHandler=UIHandler;var self=this;this.sentFileOffers={};this.acceptedFileOffers={};this.arrayToStoreChunks={};this.hasToRefresh=true;this.uploadFiles=function($scope,inputId){var fileContainer=document.getElementById(inputId);var files;var connectionId;if(uiHandler.isowner){var userindex=parseInt(inputId.split("userfiles_")[1]);if(!uiHandler.users[userindex].files){uiHandler.users[userindex].files=[]}files=uiHandler.users[userindex].files;connectionId=uiHandler.users[userindex].connectionId}else{files=uiHandler.ownerFiles;connectionId=uiHandler.ownerConnectionId}if(fileContainer){var fileOfferId=self.makeId();var fileOffer=self.sentFileOffers[fileOfferId]={};fileOffer.destinationId=connectionId;fileOffer.files={};fileOffer.attended=false;for(var i=0;i<fileContainer.files.length;i++){var fileId=self.makeId();fileOffer.files[fileId]={id:fileId,file:fileContainer.files[i]};files.push({id:fileId,name:fileContainer.files[i].name,completed:0,direction:"upload",requestId:fileOfferId})}rtc.askForAcceptFiles($scope.global.roomId,fileOfferId,fileOffer);$("#"+inputId).val("")}};this.sendFile=function($scope,userindex,ofile,requestId,token){var connectionId={};if(typeof userindex==="string"){connectionId=userindex}else{connectionId=uiHandler.users[userindex].connectionId}if(typeof userindex==="string"){if(userindex==="owner"){connectionId=uiHandler.ownerConnectionId}else{connectionId=userindex}}else{connectionId=uiHandler.users[userindex].connectionId}var reader=new window.FileReader;reader.readAsDataURL(ofile.file);reader.onload=function(event){self.onReadAsDataURL($scope,event,ofile.id,undefined,undefined,connectionId,requestId,token)}};this.onReadAsDataURL=function($scope,event,id,text,chunks,connectionId,requestId,token){var data={};var chunkLength=1e4;var orinalChunks=chunks;var remainigChunks=0;if(event)text=event.target.result;if(self.sentFileOffers[requestId].files[id].canceled){return}if(text.length>chunkLength){remainigChunks=Math.floor(text.length/chunkLength)+(text.length%chunkLength>0?1:0);data.message=text.slice(0,chunkLength)}else{remainigChunks=1;data.message=text}if(!orinalChunks)orinalChunks=remainigChunks;data.fileid=id;data.remainigChunks=remainigChunks-1;data.chunks=orinalChunks;rtc.sendMessage(connectionId,"filedata",data,requestId,token);$scope.getFile(uiHandler.isowner?$scope.getUser(connectionId).files:uiHandler.ownerFiles,id).completed=Math.floor((1-data.remainigChunks/data.chunks)*100);if(self.hasToRefresh){self.hasToRefresh=false;setTimeout(function(){uiHandler.safeApply($scope,function(){});self.hasToRefresh=true},2e3)}var remainingDataURL=text.slice(data.message.length);if(remainingDataURL.length)setTimeout(function(){self.onReadAsDataURL($scope,null,id,remainingDataURL,orinalChunks,connectionId,requestId,token)},50)};this.bytesToSize=function(bytes){var sizes=["Bytes","KB","MB","GB","TB"];if(bytes==0)return"0 Bytes";var i=parseInt(Math.floor(Math.log(bytes)/Math.log(1024)));return Math.round(bytes/Math.pow(1024,i),2)+" "+sizes[i]};this.cancelFile=function($scope,connectionId,file){if(file.direction=="upload"){rtc.cancelFile($scope.global.roomId,connectionId,file.requestId,file.id,self.sentFileOffers[file.requestId].token,"upload");self.sentFileOffers[file.requestId].files[file.id].canceled=true;file.canceled=true}else{rtc.cancelFile($scope.global.roomId,connectionId,file.requestId,file.id,self.acceptedFileOffers[file.requestId].token,"download");var data={requestId:file.requestId,id:connectionId,fileid:file.id,token:self.acceptedFileOffers[file.requestId].token,direction:"upload"};rtc.fire("file canceled",data)}};this.makeId=function(){var text="";var possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(var i=0;i<7;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text};this.dataUrlToBlob=function(dataURL){var binary=atob(dataURL.substr(dataURL.indexOf(",")+1));var array=[];for(var i=0;i<binary.length;i++){array.push(binary.charCodeAt(i))}var type;try{type=dataURL.substr(dataURL.indexOf(":")+1).split(";")[0]}catch(e){type="text/plain"}return new Blob([new Uint8Array(array)],{type:type})};this.init=function($scope){uiHandler.ownerFiles=[];$scope.openFileDialog=function(index){$("#userfiles_"+index).change(function(){$(this).unbind("change");self.uploadFiles($scope,this.id)});$("#userfiles_"+index).click()};$scope.cancelFile=function(connectionId,file){self.cancelFile($scope,connectionId,file)};$scope.getFile=function(files,fileid){if(files){for(var i=0;i<files.length;i++){if(files[i].id==fileid){return files[i]}}}return null};rtc.uniqueon("receive file offer",function(data){if(data.token&&data.requestId&&self.acceptedFileOffers[data.requestId]){if(self.acceptedFileOffers[data.requestId].token==data.token&&self.acceptedFileOffers[data.requestId].connectionId==data.socketId){rtc.receiveOffer(data.socketId,data.sdp,data.mediatype,data.requestId)}}});rtc.uniqueon("file canceled",function(data){var files=[];if(uiHandler.isowner){var user=$scope.getUser(data.id);if(!user.files){user.files=[]}files=user.files}else{files=uiHandler.ownerFiles}if(data.direction=="upload"){if(data.token&&data.requestId&&self.acceptedFileOffers[data.requestId]){if(self.acceptedFileOffers[data.requestId].token==data.token&&self.acceptedFileOffers[data.requestId].connectionId==data.id){if(self.acceptedFileOffers[data.requestId].files[data.fileid]){var fileToCancel=self.acceptedFileOffers[data.requestId].files[data.fileid];if(!fileToCancel.completed){fileToCancel.canceled=true;fileToCancel.completed=true;self.acceptedFileOffers[data.requestId].filesCompleted++;if(self.acceptedFileOffers[data.requestId].filesCompleted==Object.keys(self.acceptedFileOffers[data.requestId].files).length){rtc.dataChannels[data.id]["filedata_"+data.requestId].channel.close();delete self.acceptedFileOffers[data.requestId];rtc.dropPeerConnection(data.id,"filedata_"+data.requestId,false);rtc.allRequestCompleted($scope.global.roomId,data.id,data.requestId)}var renderedIndex=self.arrayToStoreChunks[data.id]["filedata_"+data.requestId][data.fileid].renderedFile;files[renderedIndex].completed=101;files[renderedIndex].canceled=true}}}}}else if(data.direction=="download"){self.sentFileOffers[data.requestId].files[data.fileid].canceled=true;var fileToCancel=$scope.getFile(files,data.fileid);fileToCancel.completed=101;fileToCancel.canceled=true}uiHandler.safeApply($scope,function(){})});rtc.uniqueon("data stream data",function(channel,connectionId,requestId,mediatype,message){var data=JSON.parse(message);var files=[];if(!uiHandler.isowner){files=uiHandler.ownerFiles}else{var user=$scope.getUser(connectionId);if(!user.files){user.files=[]}files=user.files}if(!self.arrayToStoreChunks[connectionId])self.arrayToStoreChunks[connectionId]={};if(!self.arrayToStoreChunks[connectionId][mediatype]){self.arrayToStoreChunks[connectionId][mediatype]={}}if(!self.acceptedFileOffers[requestId].files[data.fileid]){rtc.fileRequestFailed($scope.global.roomId,connectionId,requestId,"sent file is not allowed");channel.close();rtc.dropPeerConnection(connectionId,mediatype,false);delete self.acceptedFileOffers[requestId];return}if(!self.arrayToStoreChunks[connectionId][mediatype][data.fileid]){self.arrayToStoreChunks[connectionId][mediatype][data.fileid]=[];self.arrayToStoreChunks[connectionId][mediatype][data.fileid].renderedFile=files.length;var filename=self.acceptedFileOffers[requestId].files[data.fileid].name;files.push({id:data.fileid,name:filename,completed:0,direction:"download",requestId:requestId})}self.arrayToStoreChunks[connectionId][mediatype][data.fileid].push(data.message);var renderedIndex=self.arrayToStoreChunks[connectionId][mediatype][data.fileid].renderedFile;if(self.acceptedFileOffers[requestId].files[data.fileid].processedChunks==0){self.acceptedFileOffers[requestId].files[data.fileid].totalChunks=data.chunks}self.acceptedFileOffers[requestId].files[data.fileid].processedChunks++;if(!(data.chunks==self.acceptedFileOffers[requestId].files[data.fileid].totalChunks)||!(data.chunks-data.remainigChunks==self.acceptedFileOffers[requestId].files[data.fileid].processedChunks)){rtc.fileRequestFailed($scope.global.roomId,connectionId,requestId,"data sequence not valid");channel.close();rtc.dropPeerConnection(connectionId,mediatype,false);delete self.acceptedFileOffers[requestId];return}if(data.remainigChunks==0){var dataUrl="";files[renderedIndex].completed=100;uiHandler.safeApply($scope,function(){});rtc.fileDownloaded($scope.global.roomId,connectionId,requestId,data.fileid);while(self.arrayToStoreChunks[connectionId][mediatype][data.fileid].length>0){dataUrl=dataUrl+self.arrayToStoreChunks[connectionId][mediatype][data.fileid].shift()}self.acceptedFileOffers[requestId].files[data.fileid].completed=true;self.acceptedFileOffers[requestId].filesCompleted++;var filename=self.acceptedFileOffers[requestId].files[data.fileid].name;if(self.acceptedFileOffers[requestId].filesCompleted==Object.keys(self.acceptedFileOffers[requestId].files).length){channel.close();delete self.acceptedFileOffers[requestId];rtc.dropPeerConnection(connectionId,mediatype,false);rtc.allRequestCompleted($scope.global.roomId,connectionId,requestId)}uiHandler.safeApply($scope,function(){files[renderedIndex].completed=101});var blob=self.dataUrlToBlob(dataUrl);uiHandler.safeApply($scope,function(){files[renderedIndex].completed=102});var virtualURL=(window.URL||window.webkitURL).createObjectURL(blob);files[renderedIndex].name=$sce.trustAsHtml('<a target="_blank" download="'+filename+'	" href="'+virtualURL+'" >'+filename+"</a>");self.arrayToStoreChunks[connectionId][mediatype][data.fileid]=[];self.hasToRefresh=true}else{files[renderedIndex].completed=Math.floor((1-data.remainigChunks/data.chunks)*100)}if(self.hasToRefresh){self.hasToRefresh=false;setTimeout(function(){uiHandler.safeApply($scope,function(){});self.hasToRefresh=true},2e3)}});rtc.uniqueon("request_for_accept_files",function(data){if(data.files&&data.requestId){var confirmationStr="<strong>"+(uiHandler.isowner?uiHandler.name:$scope.getUserName(data.id))+"</strong> "+$scope.resourceBundle["wantstosharefiles"]+"<br/>";for(fileId in data.files){var curFile=data.files[fileId];curFile.processedChunks=0;confirmationStr=confirmationStr+"<li>"+curFile.name+" ("+self.bytesToSize(curFile.size)+") </li>"}if(!self.acceptedFileOffers[data.requestId]){uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:confirmationStr,yes:function(index){uiHandler.modals.splice(index,1);var fileOffer=self.acceptedFileOffers[data.requestId]={};fileOffer.token=self.makeId();fileOffer.connectionId=data.id;fileOffer.files=data.files;fileOffer.filesCompleted=0;rtc.acceptFilesRequest($scope.global.roomId,data.id,data.requestId,fileOffer.token)},no:function(index){uiHandler.modals.splice(index,1);rtc.reportErrorToUser($scope.global.roomId,data.id,data.source)},"class":"modalform editable",done:false})})}}});rtc.uniqueon("files accepted",function(data){var fileOffer=self.sentFileOffers[data.requestId];if(fileOffer&&fileOffer.attended==false&&fileOffer.destinationId==data.id){fileOffer.attended=true;
fileOffer.token=data.token;for(fileId in fileOffer.files){self.sendFile($scope,fileOffer.destinationId,fileOffer.files[fileId],data.requestId,data.token)}}});rtc.uniqueon("file downloaded",function(data){});rtc.uniqueon("files request completed",function(data){delete rtc.dataChannels[data.id]["filedata_"+data.requestId];rtc.dropPeerConnection(data.id,"filedata_"+data.requestId,true)});rtc.uniqueon("files request error",function(data){delete rtc.dataChannels[data.id]["filedata_"+data.requestId];rtc.dropPeerConnection(data.id,"filedata_"+data.requestId,true);var errMessage=$scope.getUserName(data.connectionId)+$scope.resourceBundle["error_sharefiles"];$scope.global.showError($scope,errMessage)})}}}]);angular.module("mean.system").factory("Global",[function(){var _this=this;_this._data={user:window.user,authenticated:!!window.user,roomId:"",isValidSender:function(){return navigator.webkitGetUserMedia||navigator.mozGetUserMedia},isValidReceiver:function(){return navigator.webkitGetUserMedia||navigator.mozGetUserMedia},userColors:{},styleFor:function(sender){var val=this.userColors[sender];if(!val){val="#"+Math.floor(Math.random()*16777215).toString(16);this.userColors[sender]=val}return{color:val}},namePattern:/^[^'"|]{3,30}$/,numberResolutionPattern:/^\d\d\d\d?$/,bitratePattern:/^\d\d?\d?\d?$/,bot:"||@@||",own:"||##||",showError:function(scope,err){scope.error_class="error_now";scope.error_message=err;uiHandler.safeApply(scope,function(){});setTimeout(function(){scope.error_class="";scope.error_message="";uiHandler.safeApply(scope,function(){})},1e4)},hideError:function(scope){scope.error_class="";scope.error_message=""}};return _this._data}]);angular.module("mean.rooms").factory("MediaService",["Rooms","UIHandler",function(Rooms,UIHandler){var RATIO_4_3=0;var RATIO_16_9=1;return function(){var uiHandler=UIHandler;var self=this;var room=new Rooms({});this.screen_constraints={mandatory:{chromeMediaSource:"screen",maxHeight:600,maxWidth:800},optional:[]};var constraints={audio:false,video:this.screen_constraints};var cam_constraints={audio:false,video:true};var audio_constraints={audio:true,video:false};this.mediasources={screen:{recording:false,stream:undefined,constraints:constraints,playtype:"video",winratio:RATIO_4_3,winscale:.5},video:{recording:false,stream:undefined,constraints:cam_constraints,playtype:"video",winratio:RATIO_16_9,winscale:.25},audio:{recording:false,stream:undefined,constraints:audio_constraints,playtype:"audio"}};this.isAnythingRecording=function(){return self.mediasources.screen.recording||self.mediasources.audio.recording||self.mediasources.video.recording};this.possibleResolutions=[{x:800,y:600},{x:1024,y:768},{x:1280,y:720},{x:1280,y:768},{x:1280,y:800},{x:1280,y:960},{x:1280,y:1024},{x:1366,y:768},{x:1440,y:900},{x:1680,y:900},{x:1680,y:1050},{x:1910,y:1200}];this.resolutions=[];this.receivedStreams=[];self.mute=function(stream,mute){if(stream){var audioTracks=stream.getAudioTracks();audioTracks[0].enabled=!mute}};self.startRecording=function($scope,windowHandler,source,onrecord,onclose){var startRecordingFn=function(){var mediasource=self.mediasources[source];rtc.createStream(source,mediasource.constraints,function(stream){mediasource.recording=true;mediasource.onclose=onclose;mediasource.stream=stream;if(mediasource.playtype=="video"){var mediaElement=$('<video id="my_'+source+'" style="display:none;" autoplay muted ></video>');windowHandler.create($scope,mediaElement,$scope.resourceBundle["wintitle"+source],source,mediasource.winratio,mediasource.winscale,true,function(win){mediasource.window=win;rtc.attachStream(mediasource.stream,"my_"+source);$(mediaElement).show();if(typeof onrecord!=="undefined"&&onrecord!=null){onrecord.call(self)}uiHandler.safeApply($scope,function(){})},function(win){uiHandler.safeApply($scope,function(){self.stopMedia($scope,source)})})}else if(mediasource.playtype=="audio"){if(onrecord){onrecord.call(self)}uiHandler.safeApply($scope,function(){})}},function(error){mediasource.recording=false;var errmsg="";if(source==="screen"){if(error==="installed-disabled"){errmsg=$scope.resourceBundle["loowid-extension-activate"]}else if(error==="not-installed"){errmsg=$scope.resourceBundle["loowid-extension-message"]+"<a target='_blank'  href='https://chrome.google.com/webstore/detail/loowid-screen-capturing/"+rtc.chromeDesktopExtensionId+"' >"+$scope.resourceBundle["loowid-extension-install"]+"  </a>"}}else{errmsg=$scope.resourceBundle["unablepermission"+source]+(source==="screen"?$scope.resourceBundle["readmore"]:"")}if(errmsg!=="")$scope.global.showError($scope,errmsg);if(uiHandler.access.moderated){rtc.reportErrorToOwner($scope.global.roomId,source,"cantaccess"+source)}})};if(uiHandler.isowner||!uiHandler.access.moderated){startRecordingFn()}else{uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:"<strong>"+uiHandler.ownerName+"</strong>"+$scope.resourceBundle["wantsyoushare"+source],yes:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)});startRecordingFn()},no:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)});rtc.reportErrorToOwner($scope.global.roomId,source,"denied"+source)},"class":"modalform editable",done:false,avatar:uiHandler.ownerAvatar})})}};self.stopRecording=function($scope,source){var mediasource=this.mediasources[source];if(mediasource.recording){if(mediasource.playtype=="video"){mediasource.window.winHandler.close()}else if(mediasource.playtype=="audio"){uiHandler.safeApply($scope,function(){self.stopMedia($scope,source)})}}};self.stopMedia=function($scope,source){var mediasource=this.mediasources[source];mediasource.recording=false;rtc.removeStream($scope.global.roomId,source);if(!this.isAnythingRecording())uiHandler.status="STOPPED";if(mediasource.onclose){mediasource.onclose.call(self)}mediasource.onclose=null;mediasource.stream=null};this.init=function($scope,windowHandler){uiHandler.resolutions=self.resolutions;uiHandler.currentResolution=0;uiHandler.showResolutionMenu=false;uiHandler.isMuted=false;uiHandler.modals=[];uiHandler.tutorials=[];uiHandler.canShareDesktop=navigator.webkitGetUserMedia!==undefined;var addedResolution={x:0,y:0};var resolutions={};for(el in self.possibleResolutions){if(self.possibleResolutions[el].x<=screen.width&&self.possibleResolutions[el].y<=screen.height){addedResolution=self.possibleResolutions[el];self.resolutions.push(addedResolution)}else{if(addedResolution.x!=screen.width||addedResolution.y!=screen.height)self.resolutions.push({x:screen.width,y:screen.height});break}}$scope.muteAudio=function(){uiHandler.isMuted=true;if(self.mediasources["audio"].recording){self.mute(self.mediasources["audio"].stream,true)}};$scope.unmuteAudio=function(){uiHandler.isMuted=false;if(self.mediasources["audio"].recording){self.mute(self.mediasources["audio"].stream,false)}};$scope.changeToResolution=function(index){if(self.resolutions[index]){uiHandler.currentResolution=index;var csource=self.screen_constraints.mandatory;csource.maxWidth=self.resolutions[index].x;csource.maxHeight=self.resolutions[index].y}};$scope.changeToResolution(self.resolutions.length-1);$scope.isRecording=function(source){return self.mediasources[source].recording};$scope.startRecording=function(source){self.startRecording($scope,windowHandler,source,uiHandler.isowner?function(){room.changeRoomStatus($scope.global.roomId,"BROADCASTING",function(){uiHandler.status="BROADCASTING";rtc.updateOwnerData($scope.global.roomId,uiHandler.name,uiHandler.avatar,uiHandler.status,uiHandler.access)})}:null,uiHandler.isowner?function(){if(uiHandler.status=="STOPPED"){room.changeRoomStatus($scope.global.roomId,"STOPPED",function(){rtc.updateOwnerData(uiHandler.roomId,uiHandler.name,uiHandler.avatar,uiHandler.status,uiHandler.access)})}}:null)};$scope.showDesktopAlertMessage=function(){uiHandler.safeApply($scope,function(){if(!uiHandler.tutorials)uiHandler.tutorials=[];uiHandler.tutorials.push({text:$scope.resourceBundle["justchrome"],ok:function(index){uiHandler.tutorials.splice(index,1)},left:100,top:75,"class":"tutorialform",done:false})})};$scope.changeWindowName=function(connectionId,name){if(self.receivedStreams.length>0){for(var i=0;i<self.receivedStreams.length;i++){if(self.receivedStreams[i].connectionId==connectionId){if(self.receivedStreams[i].window)self.receivedStreams[i].window.title=name}}}};$scope.stopRecording=function(source){self.stopRecording($scope,source)};$scope.askForSharing=function(connectionId,share){var askForSharingFn=function(){room.askForSharing($scope.global.roomId,uiHandler.isowner?connectionId:rtc._me,share,function(rdo){if(rdo.success){if(uiHandler.isowner){if(!uiHandler.userStatus[connectionId]){uiHandler.userStatus[connectionId]={}}uiHandler.userStatus[connectionId][share]=true}rtc.askForScreen($scope.global.roomId,connectionId,share)}})};if(uiHandler.isowner){askForSharingFn()}else{uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:$scope.resourceBundle["requestownerfor"+share],yes:function(index){uiHandler.modals.splice(index,1);askForSharingFn()},no:function(index){uiHandler.modals.splice(index,1)},"class":"modalform editable",done:false})})}};$scope.askForStopSharing=function(connectionId,origin){if(uiHandler.isowner){room.askForStopSharing(uiHandler.roomId,connectionId,origin,function(rdo){if(rdo.success){uiHandler.userStatus[connectionId][origin]=false;rtc.askForStopScreen($scope.global.roomId,connectionId,origin)}})}};rtc.uniqueon("add remote stream",function(stream,connectionId,mediatype){var mediasource={};mediasource.stream=stream;mediasource.connectionId=connectionId;mediasource.type=mediatype;mediasource.recording=true;mediasource.playtype=self.mediasources[mediatype]?self.mediasources[mediatype].playtype:"unknow";self.receivedStreams.push(mediasource);var streamId=connectionId+"_"+mediatype;if(mediasource.playtype=="video"){mediasource.winratio=self.mediasources[mediatype].winratio;mediasource.winscale=self.mediasources[mediatype].winscale;var mediaElement=$('<video id="remote_'+streamId+'" style="display:none;" autoplay muted></video>');rtc.attachStream(mediasource.stream,mediaElement.get(0));setTimeout(function(){windowHandler.create($scope,mediaElement,$scope.getUserName(connectionId),streamId,mediasource.winratio,mediasource.winscale,uiHandler.isOwner,function(win){mediasource.window=win;$(mediaElement).show();mediaElement.get(0).play();setTimeout(function(){win.height=$(mediaElement).height()+20},400);if(typeof onrecord!=="undefined"){onrecord.call(self)}uiHandler.safeApply($scope,function(){})},function(win){if(uiHandler.isowner){uiHandler.safeApply($scope,function(){$scope.askForStopSharing(connectionId,mediasource.type)})}})},1e3)}else{var mediaElement=$('<audio id="remote_'+streamId+'" style="display:none;" autoplay ></audio>');$("#remoteAudios").append(mediaElement);rtc.attachStream(mediasource.stream,"remote_"+streamId);mediaElement.get(0).play()}uiHandler.safeApply($scope,function(){})});rtc.uniqueon("stream_closed",function(data){for(var i=0;i<self.receivedStreams.length;i++){var mediasource=self.receivedStreams[i];var streamId=mediasource.connectionId+"_"+mediasource.type;if(mediasource.stream&&streamId===data.connectionId+"_"+data.mediatype){rtc.dropPeerConnection(data.connectionId,data.mediatype,false);if(mediasource.playtype==="video"){mediasource.window.winHandler.close()}else{$("#remote_"+streamId).remove();$scope.askForStopSharing(data.connectionId,mediasource.type)}self.receivedStreams.splice(i,1);break}}});rtc.uniqueon("disconnect stream",function(connectionId){if(connectionId){for(var i=0;i<uiHandler.users.length;i++){if(connectionId===uiHandler.users[i].connectionId){for(var j=0;j<self.receivedStreams.length;j++){var mediasource=self.receivedStreams[j];if(mediasource.connectionId==connectionId){var streamId=mediasource.connectionId+"_"+mediasource.type;rtc.dropPeerConnection(connectionId,mediasource.type,false);if(mediasource.playtype=="video"){mediasource.window.winHandler.close()}else{$("#remote_"+streamId).remove()}self.receivedStreams.splice(j,1);j--}}}}}});rtc.uniqueon("share_request",function(data){if(uiHandler.isowner){uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:"<strong>"+$scope.getUserName(data.id)+"</strong>"+$scope.resourceBundle["requestfor"+data.source],yes:function(index){uiHandler.modals.splice(index,1);$scope.askForSharing(data.id,data.source)},no:function(index){uiHandler.modals.splice(index,1);rtc.reportErrorToUser($scope.global.roomId,data.id,data.source)},"class":"modalform editable",done:false,avatar:$scope.getUser(data.id).avatar})})}else{$scope.startRecording(data.source)}});rtc.uniqueon("stop_request",function(data){if(uiHandler.isowner){if(uiHandler.userStatus[data.connectionId]&&uiHandler.userStatus[data.connectionId][data.source]){uiHandler.userStatus[data.connectionId][data.source]=false}}else{if(data.connectionId===rtc._me){$scope.stopRecording(data.source)}}});rtc.uniqueon("error_produced",function(data){if(uiHandler.isowner&&uiHandler.userStatus[data.connectionId]&&uiHandler.userStatus[data.connectionId][data.origin]){uiHandler.userStatus[data.connectionId][data.origin]=false;var errMessage=$scope.getUserName(data.connectionId)+$scope.resourceBundle[data.type];$scope.global.showError($scope,errMessage)}else if(uiHandler.isowner){var errMessage=$scope.getUserName(data.connectionId)+" "+$scope.resourceBundle["deny"+data.type+"request"];$scope.global.showError($scope,errMessage)}uiHandler.safeApply($scope,function(){})})}}}]);angular.module("mean.rooms").factory("Rooms",["$resource","$http","$window",function($resource,$http,$window){return function(){var csrf=window.csrf;var room=$resource("/rooms/:roomId/:connectionId/:cmd",{roomId:"@id",connectionId:"@cid"},{create:{method:"POST",params:{cmd:"create"},headers:{"x-csrf-token":csrf}},createid:{method:"POST",params:{cmd:"createid"},headers:{"x-csrf-token":csrf}},join:{method:"POST",params:{cmd:"join"},headers:{"x-csrf-token":csrf}},users:{method:"POST",params:{cmd:"users"},isArray:true,headers:{"x-csrf-token":csrf}},editOwnerName:{method:"POST",params:{cmd:"editName"},headers:{"x-csrf-token":csrf}},editShared:{method:"POST",params:{cmd:"editShared"},headers:{"x-csrf-token":csrf}},changeRoomStatus:{method:"POST",params:{cmd:"changeRoomStatus"},headers:{"x-csrf-token":csrf}},editGuestName:{method:"POST",params:{cmd:"editName"},headers:{"x-csrf-token":csrf}},isRoomAvailable:{method:"POST",params:{cmd:"isActive"},headers:{"x-csrf-token":csrf}},isRoomJoinable:{method:"POST",params:{cmd:"isJoinable"},headers:{"x-csrf-token":csrf}},askForSharing:{method:"POST",params:{cmd:"askForSharing"},headers:{"x-csrf-token":csrf}},askForStopSharing:{method:"POST",params:{cmd:"askForStopSharing"},headers:{"x-csrf-token":csrf}},moveRoom:{method:"POST",params:{cmd:"move"},headers:{"x-csrf-token":csrf}},keepSession:{method:"POST",params:{cmd:"keep"},headers:{"x-csrf-token":csrf}},claimforroom:{method:"POST",params:{cmd:"claimforroom"},headers:{"x-csrf-token":csrf}},chat:{method:"POST",params:{cmd:"chat"},headers:{"x-csrf-token":csrf}}});this.rememberUser=function(){var userName=typeof Storage!=="undefined"?localStorage.loowidUserName:null;if(!userName){userName=$window.getSuperHero()}return userName};this.makeId=function(){var text="";var possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(var i=0;i<7;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text};this.getGravatar=function(){var gravatarEmail=typeof Storage!=="undefined"?localStorage.loowidGravatarEmail:null;if(!gravatarEmail){gravatarEmail=""}return gravatarEmail};this.saveGravatar=function(gravatar){if(typeof Storage!=="undefined")localStorage.loowidGravatarEmail=gravatar};this.saveName=function(name){if(typeof Storage!=="undefined")localStorage.loowidUserName=name};this.resetName=function(){if(typeof Storage!=="undefined")localStorage.loowidUserName=""};this.getWebSocketUrl=function(){return"wss://"+window.wsocket.host+window.wsocket.port};this.getInitWebSocketUrl=function(){return location.origin.indexOf("https://"+window.wsocket.host)<0?"https://"+window.wsocket.host+"/rooms/hello":null};this.create=function(name,success,ownerToken){var gravatar=this.getGravatar();var host=this.getWebSocketUrl();this.saveName(name);var connectFunction=function(){room.createid({},function(obj){var newId=obj.id;rtc.on("connections",function(){room.create({roomId:newId,name:name,connectionId:rtc._me,avatar:$window.getGravatarImg(gravatar)},function(newRoom){success(newRoom.roomId,gravatar,$window.getGravatarImg(gravatar),newRoom.access,newRoom.dueDate)})});rtc.connect(host,newId)});return 200};var initUrl=this.getInitWebSocketUrl();if(initUrl){var wsproxyinit=$resource(initUrl,{},{hello:{method:"JSONP",params:{callback:"JSON_CALLBACK"},isArray:false}});wsproxyinit.hello({},function(){connectFunction()},function(erro){return 504})}else{connectFunction()}};this.claimforroom=function(roomId,success,failure){var connectionId=rtc._me?rtc._me:"not valid id";room.claimforroom({id:roomId,cid:connectionId},function(rdo){success(rdo)},function(err){failure(err)})};this.join=function(roomId,success){var userName=this.rememberUser();var gravatar=this.getGravatar();rtc.on("connections",function(conns){room.join({id:roomId,name:userName,connectionId:rtc._me,avatar:$window.getGravatarImg(gravatar)},success)});return{name:userName,avatar:$window.getGravatarImg(gravatar),gravatar:gravatar}};this.joinPass=function(roomId,passwdVal,reload){var host=this.getWebSocketUrl();var connectFunction=function(){rtc.connect(host,roomId,passwdVal,reload)};var initUrl=this.getInitWebSocketUrl();if(initUrl){var wsproxyinit=$resource(initUrl,{},{hello:{method:"JSONP",params:{callback:"JSON_CALLBACK"},isArray:false}});wsproxyinit.hello({},function(){connectFunction()},function(erro){return 504})}else{connectFunction()}};this.users=function(roomId,success){if(roomId){room.users({id:roomId},success)}};this.chat=function(roomId,success){this.chatPage(roomId,null,success)};this.chatPage=function(roomId,page,success){if(roomId){room.chat({id:roomId,pag:page},success)}};this.editOwnerName=function(roomId,name,gravatar,success){this.saveName(name);this.saveGravatar(gravatar);room.editOwnerName({id:roomId,name:name,avatar:$window.getGravatarImg(gravatar)},success)};this.editShared=function(roomId,acc,success){room.editShared({id:roomId,access:acc},success)};this.changeRoomStatus=function(roomId,status,success){room.changeRoomStatus({id:roomId,status:status},success)};this.editGuestName=function(roomId,name,gravatar,success){this.saveName(name);this.saveGravatar(gravatar);room.editGuestName({id:roomId,cid:rtc._me,name:name,avatar:$window.getGravatarImg(gravatar)},success)};this.getConnectionId=function(){return rtc._me};this.isRoomAvailable=function(roomId,success,failure){var connectionId=rtc._me?rtc._me:"not valid id";room.isRoomAvailable({id:roomId,cid:connectionId},success,failure)};this.isRoomJoinable=function(roomId,success,failure){room.isRoomJoinable({id:roomId},success,failure)};this.askForSharing=function(roomId,connectionId,share,success,failure){room.askForSharing({id:roomId,cid:connectionId,source:share},success,failure)};this.askForStopSharing=function(roomId,connectionId,share,success,failure){room.askForStopSharing({id:roomId,cid:connectionId,source:share},success,failure)};this.moveRoom=function(roomId,userids,success,failure){room.moveRoom({id:roomId,list:userids},success,failure)};this.keepSession=function(success,failure){room.keepSession(success,failure)}}}]);angular.module("mean.rooms").factory("UIHandler",["$window","$timeout",function($window,$timeout){var _this=this;_this._data={};_this._data.safeApply=function(scope,fn){var phase=scope.$root.$$phase;if(phase=="$apply"||phase=="$digest")scope.$eval(fn);else scope.$apply(fn)};_this._data.debug=false;_this._data.version="v"+window.loowid_version;_this._data.node=window.loowid_node;var keyboardManagerService={};var defaultOpt={type:"keydown",propagate:false,inputDisabled:false,target:$window.document,keyCode:false};keyboardManagerService.keyboardEvent={};keyboardManagerService.bind=function(label,callback,opt){var fct,elt,code,k;opt=angular.extend({},defaultOpt,opt);label=label.toLowerCase();elt=opt.target;if(typeof opt.target=="string")elt=document.getElementById(opt.target);fct=function(e){e=e||$window.event;if(opt["inputDisabled"]){var elt;if(e.target)elt=e.target;else if(e.srcElement)elt=e.srcElement;if(elt.nodeType==3)elt=elt.parentNode;if(elt.tagName=="INPUT"||elt.tagName=="TEXTAREA")return}if(e.keyCode)code=e.keyCode;else if(e.which)code=e.which;var character=String.fromCharCode(code).toLowerCase();if(code==188)character=",";if(code==190)character=".";var keys=label.split("+");var kp=0;var shift_nums={"`":"~",1:"!",2:"@",3:"#",4:"$",5:"%",6:"^",7:"&",8:"*",9:"(",0:")","-":"_","=":"+",";":":","'":'"',",":"<",".":">","/":"?","\\":"|"};var special_keys={esc:27,escape:27,tab:9,space:32,"return":13,enter:13,backspace:8,scrolllock:145,scroll_lock:145,scroll:145,capslock:20,caps_lock:20,caps:20,numlock:144,num_lock:144,num:144,pause:19,"break":19,insert:45,home:36,"delete":46,end:35,pageup:33,page_up:33,pu:33,pagedown:34,page_down:34,pd:34,left:37,up:38,right:39,down:40,f1:112,f2:113,f3:114,f4:115,f5:116,f6:117,f7:118,f8:119,f9:120,f10:121,f11:122,f12:123};var modifiers={shift:{wanted:false,pressed:e.shiftKey?true:false},ctrl:{wanted:false,pressed:e.ctrlKey?true:false},alt:{wanted:false,pressed:e.altKey?true:false},meta:{wanted:false,pressed:e.metaKey?true:false}};for(var i=0,l=keys.length;k=keys[i],i<l;i++){switch(k){case"ctrl":case"control":kp++;modifiers.ctrl.wanted=true;break;case"shift":case"alt":case"meta":kp++;modifiers[k].wanted=true;break}if(k.length>1){if(special_keys[k]==code)kp++}else if(opt["keyCode"]){if(opt["keyCode"]==code)kp++}else{if(character==k)kp++;else{if(shift_nums[character]&&e.shiftKey){character=shift_nums[character];if(character==k)kp++}}}}if(kp==keys.length&&modifiers.ctrl.pressed==modifiers.ctrl.wanted&&modifiers.shift.pressed==modifiers.shift.wanted&&modifiers.alt.pressed==modifiers.alt.wanted&&modifiers.meta.pressed==modifiers.meta.wanted){$timeout(function(){callback(e)},1);if(!opt["propagate"]){e.cancelBubble=true;e.returnValue=false;if(e.stopPropagation){e.stopPropagation();e.preventDefault()}return false}}};keyboardManagerService.keyboardEvent[label]={callback:fct,target:elt,event:opt["type"]};if(elt.addEventListener)elt.addEventListener(opt["type"],fct,false);else if(elt.attachEvent)elt.attachEvent("on"+opt["type"],fct);else elt["on"+opt["type"]]=fct};keyboardManagerService.unbind=function(label){label=label.toLowerCase();var binding=keyboardManagerService.keyboardEvent[label];delete keyboardManagerService.keyboardEvent[label];if(!binding)return;var type=binding["event"],elt=binding["target"],callback=binding["callback"];if(elt.detachEvent)elt.detachEvent("on"+type,callback);else if(elt.removeEventListener)elt.removeEventListener(type,callback,false);else elt["on"+type]=false};keyboardManagerService.bind("ctrl+shift+d",function(){rtc.debug=!rtc.debug;console.log("Debug is now "+(rtc.debug?"enabled":"disabled")+" !!")});return _this._data}]);angular.module("mean.rooms").factory("UserHandler",["Rooms","UIHandler",function(Rooms,UIHandler){return function(){var room=new Rooms({});var uiHandler=UIHandler;this.init=function($scope){uiHandler.userStatus=[];uiHandler.users=[];uiHandler.name=$scope.global.name;uiHandler.avatar=$scope.global.avatar;uiHandler.gravatar=$scope.global.gravatar;uiHandler.editName=false;uiHandler.editNameClass="";uiHandler.status=uiHandler.isowner?"OPENED":"DISCONNECTED";uiHandler.access=angular.copy($scope.global.access);$scope.changeName=function(){if(!uiHandler.name){uiHandler.name=$scope.global.name;return}if(uiHandler.isowner){room.editOwnerName($scope.global.roomId,uiHandler.name,uiHandler.gravatar,function(rdo){$scope.global.name=uiHandler.name;$scope.enableEditName();uiHandler.avatar=rdo.owner.avatar;$scope.global.avatar=uiHandler.avatar;rtc.peerListUpdated(uiHandler.roomId);rtc.updateOwnerData(uiHandler.roomId,uiHandler.name,uiHandler.avatar,uiHandler.status,uiHandler.access)})}else{room.editGuestName($scope.global.roomId,uiHandler.name,uiHandler.gravatar,function(rdo){$scope.global.name=uiHandler.name;$scope.enableEditName();var cid=room.getConnectionId();for(var i=0;i<rdo.guests.length;i++){if(rdo.guests[i].connectionId==cid){uiHandler.avatar=rdo.guests[i].avatar}}rtc.peerListUpdated($scope.global.roomId)})}};$scope.enableEditName=function(){if(!uiHandler.isowner&&uiHandler.passNeeded)return;if(!uiHandler.name){uiHandler.name=$scope.global.name;return}uiHandler.editName=!uiHandler.editName;if(uiHandler.editName){uiHandler.editNameClass="editable";setTimeout('document.getElementById("chgname").focus();',100)}else{uiHandler.editNameClass="";setTimeout('document.getElementById("chgname").blur();',100)}return false};$scope.connectedUsers=function(){var cnt=0;if(uiHandler.users){for(var i=0;i<uiHandler.users.length;i++){if(uiHandler.users[i].status=="CONNECTED"){cnt++}}}return cnt};$scope.updateUsers=function(result){uiHandler.newusers=[];if(uiHandler.users){for(var i=0;i<result.length;i++){var exists=false;for(var j=0;j<uiHandler.users.length;j++){if(uiHandler.users[j].connectionId==result[i].connectionId){exists=true;uiHandler.users[j].name=result[i].name;uiHandler.users[j].avatar=result[i].avatar;if(uiHandler.users[j].status!==result[i].status){uiHandler.newusers.unshift(result[i])}uiHandler.users[j].status=result[i].status;$scope.changeWindowName(result[i].connectionId,result[i].name)}}if(!exists){uiHandler.users.push(result[i]);uiHandler.newusers.unshift(result[i]);$scope.changeWindowName(result[i].connectionId,result[i].name)}}for(var j=0;j<uiHandler.users.length;j++){var exists=false;for(var i=0;i<result.length;i++){if(uiHandler.users[j].connectionId==result[i].connectionId){exists=true}}if(!exists){uiHandler.users.slice(j,1)}}}else{uiHandler.newusers=result;uiHandler.users=result}if(uiHandler.connected_class!=""){uiHandler.conn_new="connected_now";setTimeout(function(){uiHandler.conn_new="";uiHandler.safeApply($scope,function(){})},3e3)}};$scope.isOwner=function(id){return id===uiHandler.ownerConnectionId||$scope.global.own===id};$scope.getUser=function(connectionId){if(connectionId==$scope.global.bot)return{name:"loowid",avatar:"img/icons/favicon.ico"};if(rtc._me==connectionId||connectionId==$scope.global.own&&uiHandler.isowner)return $scope.ui;if(!uiHandler.isowner&&$scope.isOwner(connectionId))return{avatar:uiHandler.ownerAvatar,name:uiHandler.ownerName};if(uiHandler.users){for(var i=0;i<uiHandler.users.length;i++){if(uiHandler.users[i].connectionId==connectionId){return uiHandler.users[i]}}}return null};$scope.toggleConnected=function(){uiHandler.connected_class=uiHandler.connected_class=="collapsed"?"":"collapsed";uiHandler.dash_conn=uiHandler.connected_class=="collapsed"?"connected_collapsed":""};$scope.getUserName=function(connectionId){if(uiHandler.isowner&&rtc._me==connectionId){return uiHandler.name}else if(!uiHandler.isowner&&$scope.isOwner(connectionId)){return uiHandler.ownerName}else{var name="unkown user";for(var i=0;i<uiHandler.users.length;i++){if(uiHandler.users[i].connectionId==connectionId){name=uiHandler.users[i].name;continue}}}return name};rtc.on("owner_data_updated",function(data){uiHandler.ownerName=data.ownerName;uiHandler.ownerAvatar=data.ownerAvatar;$scope.changeOwnerConnectionId(data.ownerCid);$scope.changeWindowName(uiHandler.ownerConnectionId,uiHandler.ownerName);uiHandler.safeApply($scope,function(){})});if(rtc._events["peer_list_updated"]){setTimeout(function(){rtc.fire("peer_list_updated")},100)}rtc.uniqueon("peer_list_updated",function(){room.users(uiHandler.roomId,$scope.updateUsers)})}}}]);angular.module("mean.rooms").factory("WindowHandler",[function(){return function(){var defaultRelations=[{x:4,y:3},{x:16,y:9}];var e=null;this.getDefaultWidth=function(winscale){return window.innerWidth*winscale};this.getDefaultHeight=function(winratio,winscale){if(window.innerWidth<=800)winscale=.75;var width=this.getDefaultWidth(winscale);return width/defaultRelations[winratio].x*defaultRelations[winratio].y};this.getCentered=function(width){return(window.innerWidth-width)/2};this.getSomeYPosition=function(){var winCount=$("wmwindow").length;return 80+20*winCount};this.create=function($scope,mediaElement,winTitle,source,winratio,winscale,closeable,onopen,onclose){var winWidth=this.getDefaultWidth(winscale);var winHeight=this.getDefaultHeight(winratio,winscale);var options={position:{x:this.getCentered(winWidth),y:this.getSomeYPosition()},size:{width:winWidth,height:winHeight},windowContainer:"moveZone",maximizeTo:"moveZone",title:winTitle,initialZIndex:500};var open=function(win){var container=win.elem.find("video").parent();win.elem.find("video").remove();container.append(mediaElement);window.winHandler=win;onopen(window)};var selWindow=function(win){};var close=function(win){onclose(window);for(i=0;i<$scope.windows.length;i++){if(window===$scope.windows[i]){$scope.windows.splice(i,1)}}setTimeout(function(){selWindow()},100)};var window={options:options,title:winTitle,mediaElement:mediaElement,close:close,closeable:closeable===undefined?false:closeable,open:open,selectwindow:selWindow};$scope.windows.unshift(window)};this.init=function($scope){$scope.windows=[]}}}]);angular.module("mean.rooms").controller("RecordController",["$scope","$routeParams","$location","Global","Rooms","$timeout","ngI18nResourceBundle","ngI18nConfig","$cookieStore","$sce","FileService","ChatService","MediaService","WindowHandler","UserHandler","UIHandler",function($scope,$routeParams,$location,Global,Rooms,$timeout,ngI18nResourceBundle,ngI18nConfig,$cookieStore,$sce,FileService,ChatService,MediaService,WindowHandler,UserHandler,UIHandler){var uiHandler=UIHandler;$scope.global=Global;$scope.ui=uiHandler;var room=new Rooms({});var fileService=new FileService;var chatService=new ChatService;var mediaService=new MediaService;var windowHandler=new WindowHandler;var userHandler=new UserHandler;uiHandler.isowner=true;document.getElementById("noscript").style.display="none";uiHandler.conn_new="";uiHandler.connected_class="";uiHandler.chat_class="";uiHandler.helpchat_class="";uiHandler.dash_conn="";uiHandler.dash_chat="";uiHandler.shareDesktopStatus="unknown";uiHandler.eror_class="";uiHandler.error_message="";uiHandler.supportedLocales=ngI18nConfig.supportedLocales;uiHandler.defaultLocale=ngI18nConfig.defaultLocale;uiHandler.basePath=ngI18nConfig.basePath;uiHandler.cache=ngI18nConfig.cache;ngI18nResourceBundle.get().success(function(resourceBundle){$scope.resourceBundle=resourceBundle});$scope.hideError=function(){$scope.global.hideError($scope)};window.onfocus=function(){uiHandler.focused=true};window.onblur=function(){uiHandler.focused=false};uiHandler.focused=true;$scope.configureRoom=function(){room.editShared($scope.global.roomId,uiHandler.access,function(rdo){$scope.global.access=angular.copy(uiHandler.access);rtc.updateOwnerData(uiHandler.roomId,uiHandler.name,uiHandler.avatar,uiHandler.status,uiHandler.access);if(uiHandler.enabledChat!=uiHandler.access.chat){chatService.alertChatStatus($scope,rdo.access.chat?"enabled":"disabled")}$scope.enableEditAccess()})};$scope.fireUser=function(index){var users=[];if(typeof index==="string"){users.push(index)}else{users.push(uiHandler.users[index].connectionId)}var fUser=function(){room.moveRoom(uiHandler.roomId,users,function(rdo){if(rdo.success){uiHandler.roomId=$scope.global.roomId=rdo.toRoomId;$location.search("r",uiHandler.roomId);uiHandler.screenurl=$location.$$protocol+"://"+$location.$$host+"/#!/r/"+$scope.global.roomId;rtc.moveRoom(uiHandler.roomId,rdo.fromRoomId,users)}})};uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];
uiHandler.modals.push({text:"<strong>"+$scope.getUserName(users[0])+"</strong> "+$scope.resourceBundle.fireuser,yes:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)});fUser()},no:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)})},"class":"modalform editable",done:false,avatar:$scope.getUser(users[0]).avatar})})};$scope.enableEditAccess=function(){uiHandler.access=angular.copy($scope.global.access);uiHandler.editAccess=!uiHandler.editAccess;uiHandler.enabledChat=uiHandler.access.chat;if(uiHandler.editAccess){uiHandler.editAccessClass="editable";setTimeout('document.getElementById("chgacctitle").focus();',100)}else{uiHandler.editAccessClass="";setTimeout('document.getElementById("chgacctitle").blur();',100)}return false};$scope.roomLeave=function(){var leaveFn=function(){rtc.reset();$scope.global.roomId="";$location.search("r",null);$location.path("/")};if(uiHandler.status=="DISCONNECTED"){leaveFn()}else{uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:$scope.resourceBundle.errorleavetheroom,yes:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)});leaveFn()},no:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)})},"class":"modalform editable",done:false})})}};$scope.lastCharsUrl=function(url){return url.substring(url.length-10)};$scope.init=function(){var rid=$location.search().r;if(!rid)rid=$scope.global.roomId?$scope.global.roomId:$routeParams.roomId;uiHandler.roomId=$scope.global.roomId=rid;uiHandler.screenurl=$location.$$protocol+"://"+$location.$$host+"/#!/r/"+$scope.global.roomId;var guy=new Guy({appendElement:document.getElementById("eyesLogo"),color:"#FFFFFF",scale:.3});window.clearInterval($scope.global.keepInterval);$scope.global.keepInterval=window.setInterval(function(){room.keepSession(function(){},function(){})},9e5);window.clearInterval($scope.global.countDownInterval);$scope.global.countDownInterval=window.setInterval(function(){var now=new Date;var dueDate=new Date($scope.global.roomDueDate);var hours=Math.floor(Math.abs(dueDate-now)/36e5);var mins=Math.floor((Math.abs(dueDate-now)-hours*36e5)/6e4);var secs=Math.floor((Math.abs(dueDate-now)-hours*36e5-mins*6e4)/1e3);uiHandler.countDown=(hours>9?hours:"0"+hours)+"h"+(mins>9?mins:"0"+mins)+"m"+(secs>9?secs:"0"+secs)+"s";uiHandler.safeApply($scope,function(){})},1e3);room.isRoomAvailable(uiHandler.roomId,function(result){uiHandler.roomStatus=result.status;userHandler.init($scope);if(uiHandler.roomStatus=="inactive"){if(result.owner){room.joinPass($scope.global.roomId,"",true);var joinUsr=room.join($scope.global.roomId,function(results){uiHandler.roomStatus="active";$scope.global.sessionclosed=false;uiHandler.users=results.guests;uiHandler.name=results.owner.name;uiHandler.avatar=results.owner.avatar;uiHandler.gravatar=results.owner.gravatar;uiHandler.status=results.status;uiHandler.access=results.access;uiHandler.gravatar=results.owner.gravatar;$scope.global.roomDueDate=results.dueDate;$scope.global.access=uiHandler.access;uiHandler.permanenturl=$location.$$protocol+"://"+$location.$$host+"/#!/r/"+uiHandler.access.permanentkey+"/claim";$scope.global.name=uiHandler.name;$scope.global.avatar=uiHandler.avatar;$scope.global.gravatar=uiHandler.gravatar;rtc.updateOwnerData(uiHandler.roomId,uiHandler.name,uiHandler.avatar,uiHandler.status,uiHandler.access);room.chat($scope.global.roomId,function(resu){uiHandler.chatPage=resu.page;chatService.init($scope,resu.chat)})});uiHandler.name=joinUsr.name;uiHandler.avatar=joinUsr.avatar;uiHandler.gravatar=joinUsr.gravatar;$scope.global.name=uiHandler.name;$scope.global.gravatar=uiHandler.gravatar;if(window.innerWidth<=800){$scope.toogleConnected();$scope.toogleChat()}}else{$location.path("r/"+$scope.global.roomId)}}else{if(result.owner||!rtc._me){$location.path("r/"+$scope.global.roomId+"/owner")}else{room.chat($scope.global.roomId,function(results){uiHandler.chatPage=results.page;chatService.init($scope,results.chat)})}}},function(error){uiHandler.roomStatus="inactive";$scope.global.roomId="";$scope.global.sessionclosed=true;$location.path("/")});fileService.init($scope);windowHandler.init($scope);mediaService.init($scope,windowHandler)}}]).config(function($sceProvider){$sceProvider.enabled(true)}).directive("ngEscape",function(){return function(scope,element,attrs){element.bind("keydown keypress",function(event){})}});angular.module("mean.rooms").controller("RoomsController",["$scope","$routeParams","$location","Global","Rooms","$timeout","ngI18nResourceBundle","ngI18nConfig","$document","UIHandler",function($scope,$routeParams,$location,Global,Rooms,$timeout,ngI18nResourceBundle,ngI18nConfig,$document,UIHandler){var uiHandler=UIHandler;$scope.ui=uiHandler;$scope.global=Global;$scope.ui.question="none";document.getElementById("noscript").style.display="none";var room=new Rooms({});uiHandler.supportedLocales=ngI18nConfig.supportedLocales;uiHandler.defaultLocale=ngI18nConfig.defaultLocale;uiHandler.basePath=ngI18nConfig.basePath;uiHandler.cache=ngI18nConfig.cache;uiHandler.step=1;ngI18nResourceBundle.get().success(function(resourceBundle){$scope.resourceBundle=resourceBundle});uiHandler.liveRooms=[];uiHandler.liveRoomsCurrent=0;$scope.gotoRoom=function(id){$location.path("r/"+id)};$scope.create=function(){if($scope.termsaccepted||$scope.isRoomAvailable()){document.getElementById("noscript").style.display="";$scope.global.sessionclosed=false;if($scope.isRoomAvailable()){$location.path("r/"+$scope.global.roomId+"/join")}else{uiHandler.creating=true;room.create(uiHandler.name,function(id,gav,av,acc,dueDate){$scope.global.roomId=id;$scope.global.name=uiHandler.name;$scope.global.avatar=av;$scope.global.access=acc;$scope.global.gravatar=gav;$scope.global.roomDueDate=dueDate;uiHandler.permanenturl=$location.$$protocol+"://"+$location.$$host+"/#!/r/"+$scope.global.access.permanentkey+"/claim";$location.path("r/"+id+"/join")})}}else{$scope.termsnotaccepted=true}};$scope.clickterms=function($event){var checkbox=$event.target;$scope.termsnotaccepted=!checkbox.checked};$scope.setupExternal=function(){$scope.global.roomId=uiHandler.roomId=$routeParams.roomId;document.getElementById("noscript").style.display="";$scope.global.sessionclosed=false;room.claimforroom($scope.global.roomId,function(rdo){uiHandler.creating=true;$scope.global.name=uiHandler.name;$scope.global.roomId=uiHandler.roomId=rdo.id;$location.path(rdo.url)},function(){$scope.global.roomId=uiHandler.roomId="";$location.path("/")})};uiHandler.name=room.rememberUser();$scope.resetName=function(){room.resetName();uiHandler.name=room.rememberUser()};$scope.join=function(){var roomId=$scope.global.roomId;uiHandler.screenurl=$location.$$protocol+"://"+$location.$$host+"/#!/r/"+roomId;$location.path("r/"+roomId+"/join")};$scope.isRoomAvailable=function(){return $scope.global.roomId&&$scope.global.roomId.length>0};$scope.isValidBrowser=function(){return $scope.global.isValidSender()};$scope.isSessionClosed=function(){var sessionclosed=$scope.global.sessionclosed?true:false;return sessionclosed};$scope.init=function(){var guy=new Guy({appendElement:document.getElementById("eyesLogo"),color:"#FFFFFF",scale:.4})}}]);angular.module("mean.rooms").controller("ViewDesktopController",["$scope","$routeParams","$location","Global","Rooms","$timeout","ngI18nResourceBundle","ngI18nConfig","$sce","FileService","ChatService","MediaService","WindowHandler","UserHandler","UIHandler",function($scope,$routeParams,$location,Global,Rooms,$timeout,ngI18nResourceBundle,ngI18nConfig,$sce,FileService,ChatService,MediaService,WindowHandler,UserHandler,UIHandler){var uiHandler=UIHandler;$scope.global=Global;$scope.ui=uiHandler;uiHandler.userType="remote";uiHandler.connected_class="";uiHandler.chat_class="";uiHandler.helpchat_class="";uiHandler.remote_screen_class="";uiHandler.dash_conn="";uiHandler.dash_chat="";uiHandler.isowner=false;document.getElementById("noscript").style.display="none";uiHandler.error_class="";uiHandler.error_message="";var room=new Rooms({});var userHandler=new UserHandler;var fileService=new FileService;var chatService=new ChatService;var mediaService=new MediaService;var windowHandler=new WindowHandler;uiHandler.supportedLocales=ngI18nConfig.supportedLocales;uiHandler.defaultLocale=ngI18nConfig.defaultLocale;uiHandler.basePath=ngI18nConfig.basePath;uiHandler.cache=ngI18nConfig.cache;ngI18nResourceBundle.get().success(function(resourceBundle){$scope.resourceBundle=resourceBundle});$scope.hideError=function(){$scope.global.hideError($scope)};$scope.toogleConnected=function(){uiHandler.connected_class=uiHandler.connected_class=="collapsed"?"":"collapsed";uiHandler.dash_conn=uiHandler.connected_class=="collapsed"?"connected_collapsed":""};uiHandler.focused=true;window.onfocus=function(){uiHandler.focused=true};window.onblur=function(){uiHandler.focused=false};$scope.$on("$locationChangeStart",function(obj,next,current){if(next.indexOf("/join")>0){rtc.reset()}});$scope.changeOwnerConnectionId=function(id){uiHandler.ownerConnectionId=id};$scope.roomLeave=function(){var leaveFn=function(){if(!uiHandler.passNeeded&&uiHandler.joinable)rtc.reset();$scope.global.roomId="";$location.search("r",null);$location.path("/")};if(uiHandler.status=="DISCONNECTED"||!uiHandler.joinable){leaveFn()}else{uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:$scope.resourceBundle.warningleavetheroom,yes:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)});leaveFn()},no:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)})},"class":"modalform editable",done:false})})}};$scope.addPassword=function(){if(!uiHandler.roomPassword)return;uiHandler.passNeeded=false;uiHandler.connectionError=false;uiHandler.sendingPwd=true;room.joinPass($scope.global.roomId,uiHandler.roomPassword,true)};$scope.joinResult=function(results){if(results.passfail){rtc.fire("password_failed");return}uiHandler.sendingPwd=false;uiHandler.ownerName=results.owner.name;uiHandler.ownerConnectionId=results.owner.connectionId;uiHandler.status=results.status;uiHandler.ownerAvatar=results.owner.avatar;uiHandler.users=results.guests;uiHandler.currentConnectionId=rtc._me;rtc.peerListUpdated($scope.global.roomId);uiHandler.passNeeded=false;uiHandler.connectionError=false;uiHandler.access=results.access;chatService.init($scope,results.chat);for(var i=0;i<results.guests.length;i++){if(results.guests[i].connectionId===rtc._me){uiHandler.name=results.guests[i].name;uiHandler.avatar=results.guests[i].avatar}}};$scope.init=function(){if($routeParams.accessKey){uiHandler.roomPassword=$routeParams.accessKey;uiHandler.passNeeded=false;uiHandler.connectionError=false;uiHandler.sendingPwd=false;room.joinPass($scope.global.roomId,uiHandler.roomPassword,true)}if(!uiHandler.roomPassword)rtc.reset();var rid=$location.search().r;if(!rid)rid=$scope.global.roomId?$scope.global.roomId:$routeParams.roomId;var roomId=$scope.global.roomId=rid;$scope.global.roomId=uiHandler.roomId=roomId;$scope.isowner=false;$scope.isRoomJoinable();var guy=new Guy({appendElement:document.getElementById("eyesLogo"),color:"#FFFFFF",scale:.3});window.clearInterval($scope.global.keepInterval);$scope.global.keepInterval=window.setInterval(function(){room.keepSession(function(){},function(){})},9e5);window.clearInterval($scope.global.reloadInterval);$scope.global.reloadInterval=window.setInterval(function(){if(uiHandler.joinable==false&&uiHandler.locked==false&&uiHandler.permanent){window.location.reload()}},3e4);if($scope.isValidBrowser()){userHandler.init($scope);windowHandler.init($scope);mediaService.init($scope,windowHandler);if(window.innerWidth<=800){$scope.toogleConnected();$scope.toogleChat()}}else{$location.path("/")}rtc.on("password_failed",function(data){uiHandler.sendingPwd=false;uiHandler.passNeeded=true;uiHandler.connectionError=true;$scope.$apply();rtc._socket.close();delete rtc._events["get_peers"];delete rtc._events["receive_ice_candidate"];delete rtc._events["new_peer_connected"];delete rtc._events["remove_peer_connected"];delete rtc._events["receive_offer"];delete rtc._events["receive_answer"]});rtc.on("owner_data_updated",function(data){uiHandler.status=data.status;uiHandler.access=data.access;var chatModified=uiHandler.chatstatus!=undefined&&uiHandler.chatstatus!=data.access.chat;uiHandler.chatstatus=data.access.chat;if(chatModified){chatService.alertChatStatus($scope,data.access.chat==true?"enabled":"disabled")}$scope.$apply()});fileService.init($scope);rtc.on("room_moved",function(data){rtc.room=uiHandler.roomId=$scope.global.roomId=data.room;$location.search("r",data.room)});rtc.on("room_out",function(data){uiHandler.safeApply($scope,function(){if(!uiHandler.modals)uiHandler.modals=[];uiHandler.modals.push({text:$scope.resourceBundle.youarefired,ok:function(index){uiHandler.safeApply($scope,function(){uiHandler.modals.splice(index,1)});uiHandler.status="DISCONNECTED";$scope.roomLeave()},"class":"modalform editable",done:false})})});if($scope.isValidBrowser()){var joinUsr=room.join(roomId,$scope.joinResult);uiHandler.name=joinUsr.name;uiHandler.avatar=joinUsr.avatar;uiHandler.gravatar=joinUsr.gravatar;uiHandler.access=joinUsr.access;$scope.global.name=uiHandler.name;$scope.global.gravatar=uiHandler.gravatar}else{$location.path("/")}};$scope.isValidBrowser=function(){return $scope.global.isValidReceiver()};$scope.isRoomJoinable=function(){room.isRoomJoinable($scope.global.roomId,function(result){uiHandler.joinable=result.joinable;uiHandler.locked=result.locked;uiHandler.permanent=result.permanent;if(result.joinable){uiHandler.passNeeded=result.private;if(!result.private){room.joinPass($scope.global.roomId,"",true)}}},function(err){$scope.global.roomId="";uiHandler.joinable=false;$location.path("/")})}}]).config(function($sceProvider){$sceProvider.enabled(true)}).directive("ngEscape",function(){return function(scope,element,attrs){element.bind("keydown keypress",function(event){})}});
//# sourceMappingURL=loowid.min.js.map